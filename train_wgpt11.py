...
def gradient_penalty(disc, interp_x):
    ...
    gradients_w_pen = ((gradients_x[:,5:].flatten(1,2).norm(2, dim=1) - 1)**2).mean() / 3606.
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        fake_x = torch.cat([fake_x, fake_wg], dim=1)
        #fake_p = fake_x[:,:3]
        fake_xy = fake_x[:,3:5]
        ...
        interpolates_p = torch.cat([real_p[:,:,:seq_stop], fake_x[:,:3,seq_stop:]], dim=2)
