...
for e in range(n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        if (i % n_critic == 0):
            ...
            #print(torch.mean(fake_w.sum(dim=2).var(1)))
            #print(torch.mean(real_w.sum(dim=2).var(1)))
            ...
            occupancy_loss = torch.mean((fake_w.sum(2).mean(0) - real_w.sum(2).mean(0))**2)
            critic_loss = -torch.mean(out_fake)
            G_loss = critic_loss + 1.0 * occupancy_loss    
