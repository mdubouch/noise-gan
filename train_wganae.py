...
optimizer_ae = torch.optim.Adam(list(gen.dec.parameters()) + list(disc.enc.parameters()), lr=1e-3)
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        # x is a sample of wires in one-hot format. Just use real_w?
        ...
        enc = disc.enc(real_w_ohe.detach())
        #print(enc.shape)
        dec = gen.dec(enc)
        #print(dec[0])
        #print(real_w[0])
        ...
        #ae_loss = F.binary_cross_entropy(dec, real_w.detach())
        ae_loss_fn = nn.BCEWithLogitsLoss()
        ae_loss = ae_loss_fn(dec, real_w_ohe)

        #ae_loss = 1-F.cosine_similarity(dec, real_w.detach(), dim=1).mean()
        ae_loss.backward()
        optimizer_ae.step()
        ...
        #real_w = to_device(F.one_hot(real_w, num_classes=gu.cum_n_wires[-1]).squeeze(1).permute(0, 2, 1).float()).requires_grad_(True)
        #real_wx = wire_to_xy[:,real_w].squeeze(2).permute(1, 0, 2).requires_grad_(True)
