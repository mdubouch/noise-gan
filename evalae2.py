...
def sample_fake(batch_size, tau):
    ...
    dec_w = ae.dec(w)
    dec_w = F.gumbel_softmax(dec_w, dim=1, tau=tau, hard=True)
...
print('max dist real:', real_dist_matrix.max())
...
only_walls(ax)
plt.savefig(output_dir+'gen_scatter.png')
plt.close()
...
ax.set_aspect(1.0)
ax.add_line(scatter_l)
plt.savefig(output_dir+'real_scatter.png')
plt.close()
...
plt.hist(real_dist, bins=50, alpha=0.7);
plt.hist(fake_dist, bins=50, alpha=0.7);
...
for i in range(n):
    with torch.no_grad():
        ...
        w = F.gumbel_softmax(ae.dec(w), dim=1, tau=tau, hard=True)
