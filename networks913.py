...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convgeom1 = DBlock(n64*2, n64*3, 3, 1, 1, padding_mode='circular')
        self.convgeom2 = DBlock(n64*3, n64*4, 3, 1, 2, dilation=2, padding_mode='circular')

        self.conv1 = DBlock(n64*5, n64*6, 5, 4, 1, padding_mode='circular')
        self.conv2 = DBlock(n64*6, n64*8, 5, 4, 1, dilation=2, padding_mode='circular')
        self.conv3 = DBlock(n64*8, n64*10, 5, 4, 1, dilation=3, padding_mode='circular')
        self.conv4 = DBlock(n64*10, n64*12, 5, 4, 1, dilation=4, padding_mode='circular')

        self.lin0 = nn.Linear(n64*12, 1)
        self.dropout = nn.Dropout(0.1)
    ...
    def forward(self, x_, xy_, w_): 
        ...
        geom = self.convgeom1(geom)
        geom = self.convgeom2(geom)

        x0 = torch.cat([p0, geom], dim=1)

        x = self.conv1(self.dropout(x0))
        x = self.conv2(x)
        x = self.conv3(x)
        x = self.conv4(x)
