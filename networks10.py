...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len):
        ...
        self.convw2 = nn.ConvTranspose1d(ngf*2, ngf*2, 513, 1, 256)
        self.bnw2 = nn.BatchNorm1d(ngf*2)
        self.convw3 = nn.ConvTranspose1d(ngf*2, ngf*2, 513, 1, 256)
        ...
        self.convp0 = nn.ConvTranspose1d(ngf*2, ngf*2, 513, 1, 256)
        self.bnp0 = nn.BatchNorm1d(ngf*2)
        self.convp1 = nn.ConvTranspose1d(ngf*2, ngf*2, 513, 1, 256)
        self.bnp1 = nn.BatchNorm1d(ngf*2)
        self.convp2 = nn.ConvTranspose1d(ngf*2, ngf*2, 513, 1, 256)
    ...
    def forward(self, z, embed_space_noise=0.0, tau=1.0):
        ...
        x6 = self.bnu6(self.act(self.convu6(self.dropout(x5))))
        ...
        x11 = self.bn8(self.act(self.conv8(self.dropout(x10))))
        
        
        # x (batch, ngf, len)
        w0 = x11 + self.convw0(self.dropout(x11))
        w1 = w0 + self.convw1(self.dropout(self.bnw0(self.act(w0))))
        w2 = w1 + self.convw2(self.dropout(self.bnw1(self.act(w1))))
        w3 = w2 + self.convw3(self.dropout(self.bnw2(self.act(w2))))
        w4 = self.act(self.convw4(self.dropout(self.bnw3(self.act(w3)))))
        w =  self.convw((w4 + embed_space_noise * torch.randn_like(w4))).permute(0,2,1)
        
        sim = F.gumbel_softmax(w, dim=2, hard=True, tau=tau)

        p0 = x11 + self.convp0(self.dropout(x11))
        p1 = p0 + self.convp1(self.dropout(self.bnp0(self.act(p0))))
        p2 = p1 + self.convp2(self.dropout(self.bnp1(self.act(p1))))
        p3 = self.act(self.convp3(self.dropout(self.bnp2(self.act(p2)))))
        p = self.convp(p3)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len):
        ...
        self.convw3 = nn.utils.spectral_norm(nn.Conv1d(ndf*2, ndf*2, 513, 1, 256, padding_mode='circular'))
        self.convw4 = nn.utils.spectral_norm(nn.Conv1d(ndf*2, ndf*2, 513, 1, 256, padding_mode='circular'))
        ...
        self.convp1 = nn.utils.spectral_norm(nn.Conv1d(ndf*2, ndf*2, 513, 1, 256, padding_mode='circular'))
        self.convp2 = nn.utils.spectral_norm(nn.Conv1d(ndf*2, ndf*2, 513, 1, 256, padding_mode='circular'))
        self.convp3 = nn.utils.spectral_norm(nn.Conv1d(ndf*2, ndf*2, 513, 1, 256, padding_mode='circular'))
    ...
    def forward(self, p_, w_): # p_ shape is (batch, features, seq_len), w_ shape is one-hot encoded wire (batch, 4986, seq_len)
        ...
        w4 = w3 + self.convw4(self.dropout(self.act(w3)))
        w = w4
        ...
        x5 = self.conv5(self.dropout(self.act(x4)))
