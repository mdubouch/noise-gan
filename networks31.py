...
class TCNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, receptive_field_size):
        ...
        for i in range(self.n_convs):
            ...
            self.convs.append(nn.utils.spectral_norm(
                nn.Conv1d(ic, oc, 2, 1, 0, dilation=dilation).cuda()))
            self.bns.append(nn.Identity())#nn.BatchNorm1d(oc).cuda())
            self.pads.append(nn.ConstantPad1d((dilation, 0), 0.0))
            print(self.convs[i])
    ...
    def forward(self, x):
        y = x
        for i in range(self.n_convs):
            y = self.dropout(self.act(self.bns[i](self.convs[i](self.pads[i](y)))))
    ...
    def __init__(self, in_channels, out_channels, receptive_field_size):
        ...
        for i in reversed(range(self.n_convs)):
            ...
            if i < self.n_convs-1:
                ic = out_channels
            dilation = int(2**i)
            self.convs.append(nn.Conv1d(ic, oc, 2, 1, 0, dilation=dilation).cuda())
...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len):
        ...
        self.tcn1 = TCNBlock(ngf*4, ngf*2, 1024)#TCNBlockTranspose(ngf*4, ngf*2, 1024)
        ...
        self.tcnw1 = TCNBlock(ngf*2, ngf, 1024)#TCNBlockTranspose(ngf*2, ngf, 1024)
        self.convw = nn.ConvTranspose1d(ngf, n_wires, 1, 1, 0)
        ...
        self.tcnp1 = TCNBlock(ngf*2, ngf, 1024)#TCNBlockTranspose(ngf*2, ngf, 1024)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len):
        ...
        self.convw = nn.Conv1d(n_wires, ndf, 1, 1, 0)
        ...
        self.conv1 = nn.utils.spectral_norm(
                nn.Conv1d(ndf*4, ndf*6, 17, 1, 8, padding_mode='circular'))
        self.convd1 = nn.utils.spectral_norm(
                nn.Conv1d(ndf*6, ndf*6, 3, 2, 1, padding_mode='circular'))

        self.conv2 = nn.utils.spectral_norm(
            nn.Conv1d(ndf*6, ndf*6, 17, 1, 8, padding_mode='circular'))
        ...
        self.lin0 = nn.Linear(ndf*6 * seq_len // 16, 1, bias=True)
    ...
    def forward(self, p_, w_): # p_ shape is (batch, features, seq_len), w_ shape is one-hot encoded wire (batch, 4986, seq_len)
        ...
        x0 = x
        x = self.act(pool(x0) + self.convd1(x0))
