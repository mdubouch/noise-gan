...
wire_sphere = (wire_sphere) / wire_sphere.max(dim=1, keepdim=True)[0]
...
wire_grad = []
def wire_hook(grad):
    wire_grad.append(grad.abs().mean().item())
    return grad
p_grad = []
def p_hook(grad):
    p_grad.append(grad.abs().mean().item())
    return grad

def f_wireproj(x):
    return torch.tensordot(x, wire_sphere, dims=[[1], [1]]).permute(0,2,1)
...
plt.figure()
plt.plot(wire_grad)
plt.savefig(output_dir+'wire_grad.png')
plt.close()
plt.figure()
plt.plot(p_grad)
plt.savefig(output_dir+'p_grad.png')
plt.close()
