...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim, n_wires):
        ...
        self.convlink14 = nn.Conv1d(n64, n64*4, 1, 1, 0, bias=False)
    ...
    def forward(self, x_, xy_, w_): 
        ...
        x = self.act(self.conv2(self.act(x0)))
        x = self.act(self.conv3(x))

        x0 = self.conv4(x + F.interpolate(self.convlink14(x0), size=x.shape[2], mode='area'))
