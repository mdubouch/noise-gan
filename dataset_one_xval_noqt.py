...
class Data():
    ...
    def preprocess(self):
        ...
        self.minmax.fit(self.train)
        self.train_minmax = self.minmax.transform(self.train)
        self.test_minmax = self.minmax.transform(self.test)
        
    def inv_preprocess(self, tensor):
        inv_tensor = self.minmax.inverse_transform(tensor.detach().cpu().numpy())
