...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convu1 = nn.ConvTranspose1d(512, 384, 8, 8, 0)
        self.bnu1 = nn.InstanceNorm1d(384)
        self.conv0 = nn.Conv1d(384, 256, 1, 1, 0)
        self.bn0 = nn.InstanceNorm1d(256)

        self.conv1 = nn.Conv1d(256, 256, 1, 1, 0)

        self.convw = nn.Conv1d(256, n_wires, 1, 1, 0, bias=False)
        self.convp = nn.Conv1d(256, n_features, 1, 1, 0)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convxy = nn.Conv1d(2, 64, 1, 1, 0, bias=False)
        self.convp = nn.Conv1d(n_features, 64, 1, 1, 0, bias=False)
        self.convw = nn.Conv1d(n_wires, 64, 1, 1, 0, bias=False)
        self.conv1 = nn.Conv1d(64+64+64, 512, 1, 1, 0)
        self.conv2 = nn.Conv1d(512, 768, 9, 1, 4)
        self.db1 = DBlock(768, 768)
        self.db2 = DBlock(768, 768)
        ...
        self.convf = nn.Conv1d(768, 1, 1, 1, 0)
    ...
    def forward(self, x_): 
        ...
        w = self.convw(wg)
        xy = self.convxy(xy + torch.randn_like(xy) * 0.01)
        p = self.convp(p)
        #p = self.convp(p)
        #x = self.act(self.conv1(torch.cat([p, w], dim=1)))
        x = self.act(self.conv1(torch.cat([p, xy, w], dim=1)))
