...
optimizer_gen = torch.optim.Adam(list(gen.parameters()) + list(disc.enc.parameters()),  lr=1e-4, betas=(0.0, 0.9))
optimizer_disc = torch.optim.AdamW(disc.parameters(), lr=1e-4, betas=(0.0, 0.9))
optimizer_ae = torch.optim.AdamW(list(gen.dec.parameters()) + list(disc.enc.parameters()), lr=1e-4)
...
def save_states(epoch):
    states = { 'disc': disc.state_dict(), 'd_opt': optimizer_disc.state_dict(), 
            ...
            'ae_opt': optimizer_ae.state_dict(),
            ...
            'dist_mean_loss': dist_mean_losses, 'dist_var_loss': dist_var_losses }
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        enc = disc.enc(real_w_ohe)
        dec = gen.dec(enc)

        tgt = real_w_ohe.permute(0,2,1).flatten(0, 1)
        pred = dec.permute(0,2,1).flatten(0, 1)
        ae_loss = ae_loss_fn(pred, tgt, torch.ones(tgt.shape[0], device='cuda'))
        ...
        dist_loss = dist_loss_fn(fake_dist_matrix, real_dist_matrix)

        enc_loss = ae_loss + 0.1 * dist_loss
...
#        if e == 0:
        ...
        real_x = concatenate_p_w(real_p, real_w_ohe)
        ...
        real_enc_w = disc.enc(real_w_ohe)
        fake_enc_w = disc.enc(fake_w)
        ...
        interpolates_w = F.gumbel_softmax(gen.dec(interpolates_enc_w), dim=1, tau=tau, hard=True)
        ...
        if (i % n_critic == 0):
            ...
            fake_pos = disc.enc(fake_w)
            fake_dist_to_prev = torch.sqrt(1e-8 + (fake_pos[:,:,1:] - fake_pos[:,:,:-1])**2)

            real_pos = disc.enc(real_w_ohe)
            real_dist_to_prev = torch.sqrt(1e-8 + (real_pos[:,:,1:] - real_pos[:,:,:-1])**2)

            mean_loss = (fake_dist_to_prev.mean() / real_dist_to_prev.mean() - 1.)**2
            var_loss = (fake_dist_to_prev.var() / real_dist_to_prev.var() - 1.)**2
            dist_mean_losses.append(mean_loss.item())
            dist_var_losses.append(var_loss.item())

            G_loss = -torch.mean(out_fake) + mean_loss + var_loss
