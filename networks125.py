...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convw4 = nn.ConvTranspose1d(ngf*3, encoded_dim, 257, 1, 128)
    ...
    def forward(self, z, embed_space_noise=0.0, tau=1.0):
        ...
        w = self.act(w_shifted + self.convw1(x))
        w_shifted = nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1])
        w = self.act(w_shifted + self.convw2(x))
        w_shifted = nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1])
        w = self.act(w_shifted + self.convw3(x))
        w = self.convw4(w)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convw_diff1 = nn.Conv1d(encoded_dim, ndf*2, 257, 1, 128)
    ...
    def forward(self, x_): 
        ...
        w = self.act(self.convw_diff1(w - nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1])))
        w = self.act(self.convw_diff2(w - nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1])))
        w = self.act(self.convw_diff3(w - nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1])))
