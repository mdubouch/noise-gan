...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convu1 = nn.ConvTranspose1d(512, 512, 4, 4, 0)
        self.bnu1 = nn.BatchNorm1d(512)
        self.convu2 = nn.ConvTranspose1d(512, 256, 4, 4, 0)
        self.bnu2 = nn.BatchNorm1d(256)
        self.convu3 = nn.ConvTranspose1d(256, 128, 4, 4, 0)
        self.bnu3 = nn.BatchNorm1d(128)

        self.conv1 = nn.Conv1d(128, 64, 3, 1, 1)
        self.bn1 = nn.BatchNorm1d(64)

        self.convw1 = nn.Conv1d(64, 64, 3, 1, 1)
        self.bnw1 = nn.BatchNorm1d(64)
        self.convw2 = nn.Conv1d(64, 64, 17, 1, 8)
        self.bnw2 = nn.BatchNorm1d(64)
    ...
    def forward(self, z, wire_to_xy):
        ...
        x = self.act(self.bn0(self.lin0(z).view(-1, 512, self.seq_len // 64)))
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convxy2 = nn.Conv1d(64, 64, 17, 1, 8)
        self.convxy3 = nn.Conv1d(64, 64, 3, 1, 1)
        self.convw1 = nn.Conv1d(n_wires, 64, 1, 1, 0)
        self.convw2 = nn.Conv1d(64, 64, 17, 1, 8)

        self.conv1 = nn.Conv1d(192, 192, 3, 1, 1)
    ...
    def forward(self, x_): 
        ...
        xy = self.act(self.convxy2(self.dropout(xy)))
        xy = self.act(self.convxy3(self.dropout(xy)))
        p = self.act(self.convp1(p))
        p = self.act(self.convp2(self.dropout(p)))
        w = self.act(self.convw1(self.dropout(wg)))
        w = self.act(self.convw2(self.dropout(w)))
        #p = self.convp(x)
        #xy = self.convxy(xy
        ...
        x = self.dropout(self.db1(x))
        x = self.dropout(self.db2(x))
        x = self.dropout(self.db3(x))
