...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim, n_wires):
        ...
        self.convs = nn.Sequential(
                nn.ConvTranspose1d(latent_dims, 512, 4, 1, 0),
        ...
        self.convp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.ConvTranspose1d(32, 32, 3, 1, 1),
            nn.InstanceNorm1d(32),
            nn.ReLU(),
            nn.ConvTranspose1d(32, n_features, 1, 1, 0)
        )
        self.convw = nn.Sequential(
            nn.ConvTranspose1d(64, 128, 3, 1, 1),
            nn.InstanceNorm1d(128),
            nn.ReLU(),
            nn.ConvTranspose1d(128, 512, 3, 1, 1),
            nn.InstanceNorm1d(512),
            nn.ReLU(),
            nn.ConvTranspose1d(512, n_wires, 1, 1, 0)
        )
        
    def forward(self, z):
        #print('latent space %.2e %.2e' % (z.mean().item(), z.std().item()))
        # z: random point in latent space
        x = self.convs(z.view(z.shape[0], -1, 1))
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim, n_wires):
        ...
        self.convs = nn.Sequential(
                nn.Conv1d(n_features+n_wires, 64, 3, 1, 1),
                nn.LayerNorm((64,seq_len), elementwise_affine=False),
                nn.LeakyReLU(0.2),
                nn.Conv1d(64, 128, 3, 1, 1),
                nn.LayerNorm((128,seq_len), elementwise_affine=False),
                nn.LeakyReLU(0.2),
                nn.Conv1d(128, 256, 3, 1, 1),
                nn.LayerNorm((256,seq_len), elementwise_affine=False),
                nn.LeakyReLU(0.2),
