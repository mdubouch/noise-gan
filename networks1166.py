...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim, n_wires):
        ...
        self.convw = nn.Sequential(
            nn.ConvTranspose1d(8, 16, 3, 1, 1),
            nn.InstanceNorm1d(16),
            nn.ReLU(),
            nn.ConvTranspose1d(16, 32, 3, 1, 1),
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim, n_wires):
        ...
        self.emb = nn.Sequential(
            nn.Conv1d(n_wires, nproj*4, 1, 1, 0),
            nn.LeakyReLU(0.2),
            nn.Conv1d(nproj*4, nproj*2, 1, 1, 0),
            nn.LeakyReLU(0.2),
            nn.Conv1d(nproj*2, nproj*1, 1, 1, 0),
        )
