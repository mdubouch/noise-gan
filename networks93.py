...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len):
        ...
        class UpSampleResConv(nn.Module):
            def __init__(self, in_channels, out_channels):
                super().__init__()
                self.convu = nn.ConvTranspose1d(in_channels, out_channels, 4, 2, 1)
                self.conv1 = nn.Conv1d(out_channels, out_channels, 9, 1, 4)
                self.conv2 = nn.Conv1d(out_channels, out_channels, 9, 1, 4)
            ...
            def forward(self, x):
                y = self.act(self.convu(x))
                y = self.act(self.conv1(self.norm1(y)))
                y = self.act(self.conv2(self.norm2(y)) + F.interpolate(self.proj(x), scale_factor=2, 
                    mode='linear'))
                return y

        self.convu1 = UpSampleResConv(ngf*12, ngf*11)
        self.convu2 = UpSampleResConv(ngf*11, ngf*10)
        self.convu3 = UpSampleResConv(ngf*10, ngf*9)
        self.convu4 = UpSampleResConv(ngf*9, ngf*8)
        self.convu5 = UpSampleResConv(ngf*8, ngf*7)
        self.convu6 = UpSampleResConv(ngf*7, ngf*6)
        ...
        self.dec = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv1d(encoded_dim*4, encoded_dim*8, 1, 1, 0),
            nn.ReLU(),
            nn.Conv1d(encoded_dim*8, n_wires, 1, 1, 0, bias=False),
        )
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len):
        ...
        self.enc = nn.Sequential(
            nn.Conv1d(n_wires, encoded_dim*8, 1, 1, 0, bias=False),
            nn.ReLU(),
            nn.Conv1d(encoded_dim*8, encoded_dim*4, 1, 1, 0),
            nn.ReLU(),
        ...
        class DownSampleResConv(nn.Module):
            def __init__(self, in_channels, out_channels):
                super().__init__()
            ...
            def forward(self, x):
                ...
                y = self.act(self.conv2(y) + F.interpolate(self.proj(x), scale_factor=0.5, 
                    mode='linear'))
                return y

        self.convd1 = DownSampleResConv(ndf*6, ndf*7) # / 2
        self.convd2 = DownSampleResConv(ndf*7, ndf*8) # / 4
        self.convd3 = DownSampleResConv(ndf*8, ndf*9) # / 8
        self.convd4 = DownSampleResConv(ndf*9, ndf*10) # / 16
        self.convd5 = DownSampleResConv(ndf*10, ndf*11) # / 32
        self.convd6 = DownSampleResConv(ndf*11, ndf*12) # / 64
