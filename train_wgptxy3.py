...
for e in range(pretrain_epochs):
    ...
    for i in range(n_its):
        ...
        loss = ae_loss + 1 * r_loss + 1 * th_loss# + 2 * code_loss
        loss.backward()
        ...
        if e % 200 == 0 and i == 0:
            print('enc r mean', enc_r.mean().item())
            print('real r mean', wire_to_r_norm.mean().item())
            print('enc r var', enc_r.var().item())
            print('real r var', wire_to_r_norm.var().item())
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        f_w = fake_x[:,3:]
        fake_dec_w = F.gumbel_softmax(ae.dec(f_w), dim=1, hard=True, tau=tau)
        fake_enc_w = ae.enc(fake_dec_w)
        fake_x = torch.cat([fake_x[:,:3], fake_enc_w], dim=1)
