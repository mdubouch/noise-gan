...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.lin0 = nn.Linear(latent_dims, seq_len//256*ngf, bias=True)

        self.ups = nn.Upsample(scale_factor=4)

        self.conv1 = nn.Conv1d(ngf*1, ngf*2, 9, 1, 4)
        self.conv2 = nn.Conv1d(ngf*2, ngf*4, 9, 1, 4)
        self.conv3 = nn.Conv1d(ngf*4, ngf*8, 9, 1, 4)
        self.conv4 = nn.Conv1d(ngf*8, ngf*8, 9, 1, 4)
        self.bn1 = nn.InstanceNorm1d(ngf*2)
        self.bn2 = nn.InstanceNorm1d(ngf*4)
        ...
        self.convp1 = nn.ConvTranspose1d(ngf*8, ngf*1, 1, 1, 0)
        self.convp2 = nn.ConvTranspose1d(ngf*1, n_features, 17, 1, 8)

        self.convw1 = nn.ConvTranspose1d(ngf*8, ngf*4, 1, 1, 0)
        self.convw2 = nn.ConvTranspose1d(ngf*4, ngf*1, 1, 1, 0)
        self.convw3 = nn.ConvTranspose1d(ngf*1, encoded_dim, 17, 1, 8)

        self.out = nn.Tanh()
        
    def forward(self, z, embed_space_noise=0.0, tau=1.0):
        # z: random point in latent space
        x = self.act(self.lin0(z).view(-1, self.ngf*1, self.seq_len // 256))
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convp1 = nn.Conv1d(n_features, ndf*2, 17, 8, 8) # // 8
        self.convp2 = nn.Conv1d(ndf*2, ndf*3, 1, 1, 0)
        self.convp3 = nn.Conv1d(ndf*3, ndf*4, 1, 1, 0)

        self.convw1 = nn.Conv1d(encoded_dim, ndf*4, 17, 8, 8)
        self.convw2 = nn.Conv1d(ndf*4, ndf*5, 1, 1, 0)
        self.convw3 = nn.Conv1d(ndf*5, ndf*6, 1, 1, 0)

        self.convxy1 = nn.Conv1d(2, ndf*2, 17, 8, 8)
        self.convxy2 = nn.Conv1d(ndf*2, ndf*2, 1, 1, 0)
        self.convxy3 = nn.Conv1d(ndf*2, ndf*6, 1, 1, 0)

        self.conv1 = nn.Conv1d(ndf*10, ndf*8, 1, 1, 0)
        self.conv2 = nn.Conv1d(ndf*8, ndf*6, 1, 1, 0)

        self.lin0 = nn.Linear(ndf*6 * seq_len // 8, 1, bias=True)
    ...
    def forward(self, x_): 
        ...
        xy = xy - nn.ConstantPad1d((1, 0), 0.0)(xy[:,:,:-1])
