...
for i in range(gu.n_layers):
    ...
    #wire_sphere[0,start:end] = torch.from_numpy(np.arange(start, end))
    #wire_sphere[0, start:end] += torch.randn_like(wire_sphere[0, start:end]) * 5
...
print('MAX', wire_sphere.argmax(dim=1))
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        real_xy = torch.tensordot(real_w_ohe, wire_sphere, dims=[[1], [1]]).permute(0,2,1).detach()
        real_xy.requires_grad_()
        #real_xy.register_hook(print_hook)
        ...
        fake_wg.register_hook(print_hook)
        fake_xy = torch.tensordot(fake_wg, wire_sphere, dims=[[1], [1]]).permute(0,2,1).detach()
        fake_xy.requires_grad_()
        #fake_xy.register_hook(print_hook)
        ...
        #interpolates_xy = interpolates_xy + torch.randn_like(interpolates_xy) * 1.1
        ...
        print('convg0 %.2e' % (disc.convg0.weight.grad.abs().mean().item()))
        print('convw0 %.2e' % (disc.convw0.weight.grad.abs().mean().item()))
