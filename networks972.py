...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convw2 = nn.Conv1d(n512//16, n512//16, 3, 1, 1)
        self.convw1 = nn.Conv1d(n512//16, n_wires, 1, 1, 0)

        self.convp2 = nn.Conv1d(n512//16, n512//16, 3, 1, 1)
        self.convp1 = nn.Conv1d(n512//16, n_features, 1, 1, 0)
    ...
    def forward(self, z):
        ...
        w = self.act(self.convw2(self.dropout(w0)))
        w = self.convw1(w)
        ...
        p = self.act(self.convp2(self.dropout(p0)))
        p = self.convp1(p)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.bw2 = ResBlockDown(n64)
        self.bw3 = ResBlockDown(n64*2)

        self.convg1 = nn.Conv1d(geom_dim, n64, 7, 1, 3, padding_mode='circular')
        self.bg2 = ResBlockDown(n64)
        self.bg3 = ResBlockDown(n64*2)

        self.convp1 = nn.Conv1d(n_features, n64, 7, 1, 3, padding_mode='circular')
        self.bp2 = ResBlockDown(n64)
        self.bp3 = ResBlockDown(n64*2)


        # Common branch
        ncom = n64 * 12
        self.bc1 = ResBlockDown(ncom)

        self.lin0 = nn.Linear(ncom*2, 1)
        self.dropout = nn.Dropout(0.1)
