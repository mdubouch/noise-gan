...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.lin0 = nn.Linear(latent_dims, seq_len//64*n_wires, bias=True)
        ...
        self.conv1 = nn.ConvTranspose1d(n_wires, n_wires, 4, 4, 0)
        self.conv2 = nn.ConvTranspose1d(n_wires, n_wires, 4, 4, 0)
        self.conv3 = nn.ConvTranspose1d(n_wires, n_wires, 4, 4, 0)
        self.bn1 = nn.BatchNorm1d(n_wires)
        self.bn2 = nn.BatchNorm1d(n_wires)
        self.bn3 = nn.InstanceNorm1d(n_wires)

        self.convw1 = nn.ConvTranspose1d(n_wires, n_wires, 3, 1, 1)
    ...
    def forward(self, z, wire_to_xy):
        ...
        x = self.act(self.lin0(z).view(-1, n_wires, self.seq_len // 64))
        ...
        w = self.convw1(x)
        wg = F.gumbel_softmax(w, dim=1, hard=False, tau=1.0)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convpxy = nn.Conv1d(2, 64, 1, 1, 0)
        self.convw = nn.Conv1d(n_wires, 64, 1, 1, 0)
    ...
    def forward(self, x_): 
        ...
        p = self.convpxy(x[:,n_features:n_features+2])
        w = self.convw(wg)
