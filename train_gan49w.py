...
for i in range(gu.n_layers):
    ...
    #wire_sphere[0,start:end] = torch.from_numpy(np.ones_like(x) + np.random.randn(*x.shape) * 0.1)
    #wire_sphere[3,start:end] = torch.from_numpy(np.arctan2(y, x))
    wire_sphere[0,start:end] = torch.from_numpy(np.arange(start, end))
    wire_sphere[1,start:end] = torch.from_numpy(np.sqrt(x**2+y**2))
    wire_sphere[2,start:end] = torch.from_numpy(np.arctan2(y, x))
...
wire_sphere = (wire_sphere) / wire_sphere.std(dim=1, keepdim=True)
...
def gradient_penalty(disc, interp_p, interp_w):
    ...
    gradients_g = autograd.grad(outputs=d_interpolates,
            inputs=wire_sphere,
            grad_outputs=grad_outputs_w,
            create_graph=True, # IMPORTANT! Allows to compute gradient with respect to gradient
            only_inputs=True
    ...
    print(gradients_w.shape)
    print(gradients_g.shape)
    print(gradients_g)
    print(gradients_w)
    gradients_g_pen = (gradients_g.pow(2).flatten(1, 2).sum(dim=1).pow(0.5) - 1)**2
    print(gradients_g_pen.mean())

    gradient_pen = torch.mean(gradients_p_pen + gradients_w_pen)
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        out_real = disc(real_p, real_w_ohe, wire_sphere)
        ...
        out_fake = disc(fake_p, fake_wg, wire_sphere)
