...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len):
        ...
        self.resw3 = ResBlockTranspose(ngf*2, n_wires, 33, 1, 16)
        ...
        self.resp3 = ResBlockTranspose(ngf*2, n_features, 33, 1, 16)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len):
        ...
        self.resw0 = ResBlock(n_wires, ndf*2, 33, 1, 16, padding_mode='circular')
        self.resw1 = ResBlock(ndf*2, ndf*2, 17, 1, 8, padding_mode='circular')
        ...
        self.resp0 = ResBlock(n_features, ndf*2, 33, 1, 16, padding_mode='circular')
        self.resp1 = ResBlock(ndf*2, ndf*2, 17, 1, 8, padding_mode='circular')
        ...
        self.res1 = ResBlock(ndf*4, ndf*4, 33, 1, 16, padding_mode='circular')
        self.res2 = ResBlock(ndf*4, ndf*6, 17, 1, 8, padding_mode='circular')
        #self.res3 = ResBlock(ndf*4, ndf*6, 33, 1, 16, padding_mode='circular')
        #self.res4 = ResBlock(ndf*6, ndf*6, 25, 1, 12, padding_mode='circular')

        self.convd1 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 4, 2, 1, padding_mode='circular')) # // 2
        self.convd2 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 4, 2, 1, padding_mode='circular')) # // 4
        self.convd3 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 4, 2, 1, padding_mode='circular')) # // 8
        self.convd4 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 4, 2, 1, padding_mode='circular')) # // 16
        self.convd5 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 4, 2, 1, padding_mode='circular')) # // 32
        self.convd6 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 4, 2, 1, padding_mode='circular')) # // 64
    ...
    def forward(self, p_, w_): # p_ shape is (batch, features, seq_len), w_ shape is one-hot encoded wire (batch, 4986, seq_len)
        ...
        x = self.act(self.convd2(x) + F.interpolate(x0, scale_factor=1/4))

        x0 = x
        x = self.act(self.convd3(x0))
        x = self.act(self.convd4(x) + F.interpolate(x0, scale_factor=1/4))

        x0 = x
        x = self.act(self.convd5(x0))
        x = self.act(self.convd6(x) + F.interpolate(x0, scale_factor=1/4))
