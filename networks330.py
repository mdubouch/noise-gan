...
class Gen(nn.Module):
    ...
    def forward(self, z):
        ...
        w = self.linw(x.permute(0,2,1)).permute(0,2,1)
        p = self.linp(x.permute(0,2,1)).permute(0,2,1)
...
class Disc(nn.Module):
    ...
    def forward(self, x_): 
        ...
        p = self.act(self.linp(p)).permute(0,2,1)
        w = self.act(self.linw(w)).permute(0,2,1)
