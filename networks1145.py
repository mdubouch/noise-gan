    ...
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim, n_wires):
        ...
        class Res(nn.Module):
            ...
            def forward(self, x):
                ...
                y = self.s2(y)
                y = self.s3(y)
                y = self.norm4(self.act(y0 + self.conv4(y)))
                return y

        self.s1 = Res(latent_dims, 128, 5, 4, 1, output_padding=1)
        self.s2 = Res(128, 128, 3, 2, 1, output_padding=1)
        self.s3 = Res(128, 128, 3, 2, 1, output_padding=1)
        self.s4 = Res(128, 128, 3, 2, 1, output_padding=1)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim, n_wires):
        ...
        self.s1 = Res(1+n_features+geom_dim, 64, 3, 1, 1, padding_mode='circular')
        ...
        self.s7 = Res(128, 128, 3, 2, 1)
        self.s8 = Res(128, 128, 3, 2, 1)
        self.s9 = Res(128, 128, 3, 2, 1)
        self.s10 = Res(128, 128, 3, 2, 1)
    ...
    def forward(self, x_, xy_, w_): 
        ...
        x = torch.cat([xy, p, occupancy.expand(-1, 1, seq_len)], dim=1)
