...
wire_to_xy_norm = wire_to_xy / wire_to_xy.std(dim=1, keepdim=True)
...
only_walls(ax)
plt.savefig(output_dir+'gen_scatter.png', dpi=args.dpi*2)
plt.close()
...
n_seqs = 4 # Number of sequences per generated batch
...
first_idx = 0#np.random.randint(0, data.dbg_z.size - seq_len)
...
ax[1].set_aspect(1.0)
only_walls(ax[1])
plt.savefig(output_dir+'comp_scatter.png', dpi=args.dpi*2)
plt.close()
...
for i in range(n):
    with torch.no_grad():
        ...
        #w = F.gumbel_softmax(w, dim=1, hard=True, tau=0.75)
