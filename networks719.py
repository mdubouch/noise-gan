...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convw3 = nn.ConvTranspose1d(n512, n256, 12, 4, 4)
        self.bnw3 = nn.BatchNorm1d(n256)
        self.convw2 = nn.ConvTranspose1d(n256, n128, 12, 4, 4)
        self.bnw2 = nn.BatchNorm1d(n128)
        self.convw1 = nn.Conv1d(n128, n_wires, 1, 1, 0)

        self.convp3 = nn.ConvTranspose1d(n512, n256, 12, 4, 4)
        self.bnp3 = nn.BatchNorm1d(n256)
        self.convp2 = nn.ConvTranspose1d(n256, n128, 12, 4, 4)
        self.bnp2 = nn.BatchNorm1d(n128)
        self.convp1 = nn.Conv1d(n128, n_features, 1, 1, 0)
    ...
    def forward(self, z, wire_to_xy):
        ...
        x = self.act(self.bn0(self.lin0(z).reshape(-1, self.n512, self.seq_len // 16)))
        ...
        w = self.act(self.bnw3(self.convw3(x)))
        ...
        p = self.act(self.bnp3(self.convp3(x)))
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv0 = nn.utils.spectral_norm(nn.Conv1d(n_features, n64, 16, 4, 0))

        self.conv1 = nn.utils.spectral_norm(nn.Conv1d(n64, n128, 12, 4, 0))
        ...
        self.conv4 = nn.utils.spectral_norm(nn.Conv1d(n256+n512, n512, 8, 2, 0))
        self.conv5 = nn.utils.spectral_norm(nn.Conv1d(n512, n512, 4, 2, 0))
        self.conv6 = nn.utils.spectral_norm(nn.Conv1d(n512+n512, n512, 4, 2, 0))
    ...
    def forward(self, x_): 
        ...
        x0 = F.interpolate(x0, size=x1.shape[2], mode='linear')
        x2 = self.conv2(self.act(torch.cat([x0, x1], dim=1)))

        x3 = self.conv3(self.act(x2))
        x2 = F.interpolate(x2, size=x3.shape[2], mode='linear')
        x4 = self.conv4(self.act(torch.cat([x2, x3], dim=1)))

        x5 = self.conv5(self.act(x4))
        x4 = F.interpolate(x4, size=x5.shape[2], mode='linear')
