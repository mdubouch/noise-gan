...
for e in range(pretrain_epochs):
    ...
    for i in range(n_its):
        ...
        loss = ae_loss + 0.1* ae.enc_net.kl_loss + 500* dist_loss# + 10 * dist_loss + 5 * norm_loss
        #print(ae.enc_net.kl_loss)
        loss.backward()
        optimizer_ae.step()
        pretrain_losses.append(ae_loss.item())
        kld_losses.append(ae.enc_net.kl_loss.item())
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        #fake_dec_w = F.gumbel_softmax(ae.dec(fake_w), dim=1, hard=False, tau=tau)
        #fake_xy = torch.tensordot(fake_dec_w, wire_to_xy, dims=[[1], [1]]).permute(0, 2, 1)
        fake_xy = 0
        fake_enc_w = fake_w#ae.enc(fake_dec_w)
        ...
        #interpolates_dec_w = F.gumbel_softmax(ae.dec(interpolates_w), dim=1, hard=False, tau=tau)
