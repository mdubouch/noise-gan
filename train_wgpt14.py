...
wire_to_xy_norm = (wire_to_xy - wire_to_xy * (wire_to_r.min()-10) / wire_to_r) / (wire_to_r.max() - wire_to_r.min() + 50)
wire_to_xy_norm.requires_grad_(False)
print(wire_to_xy_norm.min(), wire_to_xy_norm.max())
wire_to_r_norm = (wire_to_r - wire_to_r.min() + 10) / (wire_to_r.max() - wire_to_r.min() + 10)
...
def gradient_penalty(disc, interp_x):
    d_interpolates = disc(interp_x.requires_grad_(True), wire_to_xy_norm).squeeze()
    ...
    gradients_p_pen = ((gradients_x[:,:3].flatten(1,2).norm(2, dim=1) - 1)**2).mean()
    #gradients_xy_pen = ((gradients_x[:,3:5].flatten(1,2).norm(2, dim=1) - 1)**2).mean()
    gradients_w_pen = ((gradients_x[:,3:].flatten(1,2).norm(2, dim=1) - 1)**2).mean()
    #gradients_x = gradients_x[:,:5].reshape(gradients_x.shape[0], -1) + 1e-8
    print('p_pen %.2e' % (gradients_p_pen.item()))
    ##print('xy_pen %.2e' % (gradients_xy_pen.item()))
    print('w_pen %.2e' % (gradients_w_pen.item()))
    gradient_pen = gradients_p_pen #+ gradients_w_pen
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        #enc_r = torch.norm(real_enc_w, dim=1)
        #batch_r = wire_to_r_norm[real_w].squeeze()
        #r_loss = F.mse_loss(enc_r, batch_r)
        #ae_loss = reco_loss + r_loss
        #ae_losses.append(ae_loss.item())
        #print(np.mean(ae_losses[-100:]))
        #real_xy = wire_to_xy_norm[:, real_w].squeeze(2).permute(1,0,2).requires_grad_(True)
        real_xy = torch.tensordot(real_w_ohe, wire_to_xy_norm, dims=[[1],[1]]).permute(0,2,1)

        real_x = torch.cat([real_p, real_w_ohe], dim=1)
        out_real = disc(real_x, wire_to_xy_norm)

        fake_x, fake_wg = sample_fake(real_p.shape[0], tau)
        fake_x = torch.cat([fake_x[:,:3], fake_wg], dim=1)
        #fake_p = fake_x[:,:3]
        fake_xy = fake_x[:,3:5]
        #fake_gumbel = gumbel(fake_w)
        #fake_xy = torch.tensordot(fake_gumbel, wire_to_xy_norm, dims=[[1], [1]]).permute(0,2,1)
        #fake_x = torch.cat([fake_x[:,:3], ae.enc(gumbel(ae.dec(fake_w)))], dim=1)
        #fake_x = torch.cat([fake_x[:,:3], fake_xy], dim=1)

        #print(real_xy.norm(dim=1).max().item())
        out_fake = disc(fake_x, wire_to_xy_norm)

        eps = torch.rand((real_p.shape[0], 1, 1), device='cuda')
        seq_stop = (eps[0] * seq_len).long()
        interpolates_p = (eps * real_p + (1-eps) * fake_x[:,:3]).requires_grad_(True)
        #interpolates_x = torch.cat([real_x[:,:,:seq_stop], fake_x[:,:,seq_stop:]], dim=2)
        interpolates_xy = torch.cat([real_xy[:,:,:seq_stop], fake_xy[:,:,seq_stop:]], dim=2)
        interpolates_w = torch.cat([real_w_ohe[:,:,:seq_stop], fake_wg[:,:,seq_stop:]], dim=2)
        ...
        if (critic_count % n_critic == 0):
            ...
            #enc_r = torch.norm(real_enc_w, dim=1)
            #batch_r = wire_to_r_norm[real_w].squeeze()
            #r_loss = F.l1_loss(enc_r, batch_r)
            #ae_loss = reco_loss + 10 * r_loss
            #ae_losses.append(ae_loss.item())
            #print(np.mean(ae_losses[-100:]), r_loss.item())

            fake_x, fake_wg = sample_fake(real_p.shape[0], tau)
            fake_x = torch.cat([fake_x[:,:3], fake_wg], dim=1)

            #print(fake_x[0,3:5])
            #print(wire_to_xy_norm[:,torch.argmax(fake_wg[0], dim=0)])
            #fake_w = fake_x[:,3:]
            #fake_gumbel = gumbel(fake_w)
            #fake_xy = torch.tensordot(fake_gumbel, wire_to_xy_norm, dims=[[1], [1]]).permute(0,2,1)
            #fake_w = fake_x[:,3:]

            #fake_dec = gumbel(ae.dec(fake_w))
            #fake_x = torch.cat([fake_x[:,:3], ae.enc(fake_dec)], dim=1)
            #fake_x = torch.cat([fake_x[:,:3], fake_xy], dim=1)

            out_fake = disc(fake_x, wire_to_xy_norm)

            G_loss = -torch.mean(out_fake)# + torch.mean(out_real))# + ae_loss
