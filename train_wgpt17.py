...
def sample_fake(batch_size, tau):
    ...
    sample = gen(noise, wire_to_xy_norm)
    return sample
...
def gradient_penalty(disc, interp_x):
    ...
    gradients_xy_pen = ((gradients_x[:,data.n_features:data.n_features+2].flatten(1,2).norm(2, dim=1) - 1)**2).mean()
    #gradients_w_pen = ((gradients_x[:,-gu.n_wires:].flatten(1,2).norm(2, dim=1) - 1)**2).mean()
    ...
    gradient_pen = gradients_p_pen + gradients_xy_pen
...
#print('INITIAL weight', gen.convpw.weight.mean().item(), gen.convpw.weight.std().item())
def p_hook(grad):
    print('p %.2e' % (grad.abs().mean().item()))
    return grad# * wire_to_r_norm.view(1, 3606, 1)
def wire_hook(grad):
    print('w  %.2e' % (grad.abs().mean().item()))
    return grad * wire_to_r_norm.sum()
print('wwwwwwwwwww', wire_to_r_norm.norm())
def xy_hook(grad):
    print('xy  %.2e' % (grad.abs().mean().item()))
    return grad# * wire_to_r_norm.view(1, 3606, 1)
class WireProj(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input):
        # input is gumbel_softmax output from G
        out = torch.tensordot(input, wire_to_xy_norm, dims=[[1],[1]]).permute(0,2,1)
        ctx.save_for_backward(input)
        print(input.grad)
        return out
    @staticmethod
    def backward(ctx, grad_output):
        print('xy grad %.2e' % grad_output.abs().mean().item())
        input, = ctx.saved_tensors
        print(input.grad)
        return input.grad
wireproj = WireProj.apply
class Identity(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output):
        print('identity %.2e' % grad_output.abs().mean().item())
        return grad_output
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        real_xy = wire_to_xy_norm[:,real_w].squeeze().permute(1, 0, 2)

        real_x = torch.cat([real_p, real_xy], dim=1).requires_grad_()
        out_real = disc(real_x)

        #with torch.no_grad():
        fake_p, fake_wg = sample_fake(real_p.shape[0], tau)
        #fake_wg.register_hook(wire_hook)
        fake_xy = torch.tensordot(fake_wg, wire_to_xy_norm, dims=[[1], [1]]).permute(0,2,1)
        fake_x = torch.cat([fake_p, fake_xy], dim=1)
        ...
        interpolates_p =  torch.cat([real_p[:,:,:seq_stop], 
                                     fake_p[:,:,seq_stop:].detach()], dim=2)
        interpolates_xy = torch.cat([real_xy[:,:,:seq_stop], 
                                     fake_xy[:,:,seq_stop:].detach()], dim=2)
        interpolates_w =  torch.cat([real_w_ohe[:,:,:seq_stop], 
                                     fake_wg[:,:,seq_stop:].detach()], dim=2)

        interpolates_x = torch.cat([interpolates_p, interpolates_xy], dim=1)
        ...
        if (critic_count % n_critic == 0):
            ...
            fake_p, fake_wg = sample_fake(real_p.shape[0], tau)
            fake_p = identity(fake_p)
            #fake_p.register_hook(p_hook)
            fake_wg.register_hook(wire_hook)
            # Figure out what the avg gradient is in inner and outer layers
            
            #fake_xy = wireproj(fake_wg)
            fake_xy = torch.tensordot(fake_wg, wire_to_xy_norm, dims=[[1], [1]]).permute(0,2,1)
            fake_xy.register_hook(xy_hook)
            fake_x = torch.cat([fake_p, fake_xy], dim=1)
            print('fake xy norm %.2e' % fake_xy[0,:,0].norm().item())

            print(fake_p.abs().mean().item(), fake_xy.abs().mean().item(),
                    fake_wg.abs().mean().item())
            print(fake_p.norm(dim=1).mean().item(), fake_xy.norm(dim=1).mean().item(),
                    fake_wg.norm(dim=1).mean().item())
            out_fake = disc(fake_x)
            ...
            G_loss.backward()
            print()
            print('lin0 %.2e' % gen.lin0.weight.grad.abs().mean().item())
            print('w3 %.2e' % gen.convw3.weight.grad.abs().mean().item())
            print('w2 %.2e' % gen.convw2.weight.grad.abs().mean().item())
            print('w1 %.2e' % gen.convw1.weight.grad.abs().mean().item())
            print('p3 %.2e' % gen.convp3.weight.grad.abs().mean().item())
            print('p2 %.2e' % gen.convp2.weight.grad.abs().mean().item())
            print('p1 %.2e' % gen.convp1.weight.grad.abs().mean().item())
            print('d xy1 %.2e' % disc.convxy1.weight.grad.abs().mean().item())
            print('d xy2 %.2e' % disc.convxy2.weight.grad.abs().mean().item())
            print('d p1 %.2e' % disc.convp1.weight.grad.abs().mean().item())
            print('d p2 %.2e' % disc.convp2.weight.grad.abs().mean().item())
            print('d convf %.2e' % disc.convf.weight.grad.abs().mean().item())
            print('d convf weight %.2e' % disc.convf.weight.abs().mean().item())
            #print(fake_wg.shape)
            #print(fake_wg.grad, fake_xy.grad, fake_p.grad)
