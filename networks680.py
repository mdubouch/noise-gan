...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convw4 = nn.ConvTranspose1d(n16, n16, 33, 1, 16)
        self.bnw4 = nn.BatchNorm1d(n16)
        self.convw5 = nn.ConvTranspose1d(n16, n16, 65, 1, 32)
        self.bnw5 = nn.BatchNorm1d(n16)
        self.convw6 = nn.Conv1d(n16, n_wires, 1, 1, 0, bias=True)
    ...
    def forward(self, z, wire_to_xy):
        ...
        w = self.act(self.bnw4(self.convw4(w)))
        w = self.act(self.bnw5(self.convw5(w)))
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv0 = nn.utils.spectral_norm(nn.Conv1d(geom_dim, n64, 5, 2, 0))

        self.conv1 = nn.utils.spectral_norm(nn.Conv1d(n64, n128, 5, 2, 0))

        self.conv2 = nn.utils.spectral_norm(nn.Conv1d(n128, n256, 5, 2, 0))
        self.conv3 = nn.utils.spectral_norm(nn.Conv1d(n256, n512, 5, 2, 0))
        self.conv4 = nn.utils.spectral_norm(nn.Conv1d(n512, n512, 5, 2, 0))
        self.conv5 = nn.utils.spectral_norm(nn.Conv1d(n512, n512, 5, 2, 0))
