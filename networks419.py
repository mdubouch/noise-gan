...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv0 = nn.Conv1d(n_features, 64, 1, 1, 0)
        self.convwg = nn.Conv1d(n_wires, 64, 1, 1, 0)
        self.conv1 = nn.Conv1d(128, 128, 5, 1, 2)
        self.conv2 = nn.Conv1d(128, 256, 5, 1, 2)
        self.conv3 = nn.Conv1d(256, 512, 3, 1, 1)
    ...
    def forward(self, x_): 
        ...
        x = self.act(self.conv0(p))
        xwg = self.act(self.convwg(wg))
        x = torch.cat([x, xwg], dim=1)
