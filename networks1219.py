...
class SelfAttn(nn.Module):
    ...
    def forward(self,x):
        ...
        #attn = torch.softmax(torch.bmm(Q, K), dim=2) # (B, L, L)

        out = torch.bmm(V, torch.softmax(torch.bmm(Q, K), dim=2)) # (B, C, L)
...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim, n_wires):
        ...
        n512 = 256
        self.lin0 = nn.Linear(latent_dims, seq_len//4*n512, bias=True)
    ...
    def forward(self, z):
        ...
        x = self.act(self.lin0(z).reshape(-1, self.n512, self.seq_len // 4))
