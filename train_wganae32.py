...
optimizer_gen = torch.optim.AdamW(list(gen.parameters()) + list(ae.dec.parameters()),  
        lr=1e-4, betas=(0.9, 0.99))
optimizer_disc = torch.optim.AdamW(list(disc.parameters()) + list(ae.enc.parameters()),
        lr=1e-4, betas=(0.9, 0.99))
optimizer_ae = torch.optim.AdamW(ae.parameters(), lr=1e-4, betas=(0.9, 0.99))
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        enc_loss = ae_loss + 0.2 * dist_loss
