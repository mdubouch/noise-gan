...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.ln1 = nn.LayerNorm((n128, seq_len//2))
        self.conv2 = nn.Conv1d(n64+n64+n64+n128, n256, 2, 2, 0)
        self.ln2 = nn.LayerNorm((n256, seq_len//4))
        self.conv3 = nn.Conv1d(n256, n512, 2, 2, 0)
        self.ln3 = nn.LayerNorm((n512, seq_len//8))
        self.conv4 = nn.Conv1d(n256+n512, n768, 2, 2, 0)
        self.ln4 = nn.LayerNorm((n768, seq_len//16))
        self.conv5 = nn.Conv1d(n768, n768, 4, 4, 0)
        self.ln5 = nn.LayerNorm((n768, seq_len//64))
    ...
    def forward(self, x_, w_, wire_sphere): 
        ...
        x1 = self.ln1(self.conv1(x0))
        x0 = F.interpolate(x0, size=x1.shape[2], mode='area')
        x2 = self.ln2(self.conv2(self.act(torch.cat([x0, x1], dim=1))))

        x3 = self.ln3(self.conv3(self.act(x2)))
        x2 = F.interpolate(x2, size=x3.shape[2], mode='area')
        x4 = self.ln4(self.conv4(self.act(torch.cat([x2, x3], dim=1))))

        x5 = self.ln5(self.conv5(self.act(x4)))
