...
wire_cylinder = torch.zeros((2, gu.n_wires), device='cuda')
...
for i in range(gu.n_layers):
    ...
    wire_cylinder[0,start:end] = torch.from_numpy(x)
    wire_cylinder[1,start:end] = torch.from_numpy(y)
    #wire_cylinder[2,start:end] = torch.zeros_like(z)
...
wire_sphere = wire_cylinder# / wire_cylinder.norm(dim=0, keepdim=True)
wire_radius = wire_sphere.norm(dim=0)
print('aaa', wire_radius)
wire_radius_std = wire_radius.std()
print('radius std', wire_sphere.norm(dim=0).std())
wire_sphere = wire_sphere / wire_radius_std * data_std

print('GEOM STD', wire_sphere.std(dim=1))
print('NORM', wire_sphere.norm(dim=0))
print('radius std', wire_sphere.norm(dim=0).std())
...
def gradient_penalty(disc, interp_x):
    ...
    gradients_w_pen = ((gradients_x[:,data.n_features+2:].flatten(1,2).norm(2, dim=1) - 1)**2).mean()
