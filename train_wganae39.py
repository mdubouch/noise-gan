...
logging.info(disc)
ae = to_device(networks.AE(encoded_dim=encoded_dim))
...
optimizer_gen = torch.optim.Adam(list(gen.parameters()) + list(ae.dec_net.parameters()),  
        lr=1e-4, betas=(0.0, 0.9))
optimizer_disc = torch.optim.AdamW(list(disc.parameters()) + list(ae.enc_net.parameters()),
        lr=1e-4, betas=(0.0, 0.9))
optimizer_ae = torch.optim.Adam(ae.parameters(), lr=1e-3, betas=(0.5,0.99))
...
for e in range(pretrain_epochs):
    wires = torch.tensor(
            np.random.choice(gu.n_wires, size=(gu.n_wires // 256, 256), replace=False),
            device='cuda')
    n_its = wires.shape[0]
    for i in range(n_its):
        ...
        wid = wires[i]
        # wid (256,)
        wid_ohe = F.one_hot(wid, num_classes=gu.n_wires).float().requires_grad_(True)
        # wid_ohe (256, 3606)
        enc = ae.enc_net(wid_ohe)
        # enc (256, enc_dim)
        fake_dist_matrix = enc.view(256, 1, encoded_dim) - enc.view(1, 256, encoded_dim)
        # fake_dist_matrix (256, enc_dim, enc_dim)
        fake_dist_matrix = torch.sqrt(1e-16 + (fake_dist_matrix**2).sum(dim=2))
        real_dist_matrix_sub = real_dist_matrix[wid][:, wid]
        #print(real_dist_matrix_sub.shape)
        #print(real_dist_matrix_sub)
        #print(real_dist_matrix_sub.max())
        #print(fake_dist_matrix.max())
        dist_loss = nn.MSELoss()(fake_dist_matrix, real_dist_matrix_sub)


        encdec = ae.dec_net(enc)
        # encdec (256, 3606)
        ae_loss = ae_loss_fn(encdec, wid)
    ...
    if e % 1000 == 0:
        ...
        print('accurary:', np.mean(pretrain_acc[-100:]))
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        real_w_ohe = to_device(F.one_hot(real_w, 
            num_classes=gu.cum_n_wires[-1])
                .squeeze(1).permute(0, 2, 1).float()).requires_grad_(True)
        ...
        real_x = concatenate_p_w(real_p, real_enc_w)
        out_real = disc(real_x)
        D_loss_real = out_real

        fake_p, fake_w = sample_fake(real_p.shape[0], tau)
        fake_dec_w = F.gumbel_softmax(ae.dec(fake_w), dim=1, tau=tau, hard=True)
        fake_enc_w = ae.enc(fake_dec_w)

        fake_x = concatenate_p_w(fake_p, fake_enc_w).detach()
        ...
        if (i % n_critic == 0):
            ...
            fake_dec_w = F.gumbel_softmax(ae.dec(fake_w), dim=1, tau=tau, hard=True)
            fake_enc_w = ae.enc(fake_dec_w)
            fake_x = concatenate_p_w(fake_p, fake_enc_w)
