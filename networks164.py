...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convp1 = nn.ConvTranspose1d(ngf*8, ngf*1, 3, 1, 1)
        self.convp2 = nn.ConvTranspose1d(ngf*1, n_features, 3, 1, 1)

        self.convw1 = nn.ConvTranspose1d(ngf*8, ngf*8, 3, 1, 1)
        self.convw2 = nn.ConvTranspose1d(ngf*8, encoded_dim, 3, 1, 1)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convp1 = nn.Conv1d(n_features, ndf*4, 257, 1, 128)
        self.convp2 = nn.Conv1d(ndf*4, ndf*4, 1, 1, 0)

        self.convw1 = nn.Conv1d(encoded_dim, ndf*8, 513, 1, 256)
        self.convw2 = nn.Conv1d(ndf*8, ndf*8, 1, 1, 0)

        self.conv1 = nn.Conv1d(ndf*12, ndf*12, 1, 1, 0)

        self.lin0 = nn.Linear(ndf*12 * seq_len // 1, 1, bias=True)
