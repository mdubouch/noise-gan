...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.conv1 = nn.ConvTranspose1d(n128, n64, 32, 2, 15)
        self.bn1 = nn.BatchNorm1d(n64)

        self.convw1 = nn.ConvTranspose1d(n64, n64, 64, 2, 31)
        self.convw2 = nn.ConvTranspose1d(n64, n_wires, 1, 1, 0, bias=True)

        self.convp1 = nn.ConvTranspose1d(n64, n64, 64, 2, 31)
        self.convp2 = nn.ConvTranspose1d(n64, n_features, 1, 1, 0)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv0 = nn.utils.spectral_norm(nn.Conv1d(n_features+geom_dim, n64, 3, 2, 0))

        self.conv1 = nn.utils.spectral_norm(nn.Conv1d(n64, n128, 3, 2, 0))

        self.conv2 = nn.utils.spectral_norm(nn.Conv1d(n128, n256, 3, 2, 0))
        self.conv3 = nn.utils.spectral_norm(nn.Conv1d(n256, n512, 3, 2, 0))
        self.conv4 = nn.utils.spectral_norm(nn.Conv1d(n512, n512, 3, 2, 0))
        self.conv5 = nn.utils.spectral_norm(nn.Conv1d(n512, n512, 3, 4, 0))
        self.conv6 = nn.utils.spectral_norm(nn.Conv1d(n512, n512, 3, 4, 0))
    ...
    def forward(self, x_): 
        ...
        x = self.conv4(self.act(x3))
        x = self.conv5(self.act(x))
        x = self.conv6(self.act(x))
