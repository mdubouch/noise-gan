    ...
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim, n_wires):
        ...
        class Res(nn.Module):
            def __init__(self, in_c, out_c, k_s, stride, *args, **kwargs):
                ...
                self.conv3 = nn.ConvTranspose1d(out_c, out_c, 3, 1, 1)
                self.norm3 = nn.BatchNorm1d(out_c)
                if in_c != out_c:
            ...
            def forward(self, x):
                ...
                y = self.norm1(self.act(self.conv1(x)))
                y = self.norm3(self.act(y0 + self.conv3(y)))
                return y
    ...
    def __init__(self, ndf, seq_len, encoded_dim, n_wires):
        ...
        class Res(nn.Module):
            def __init__(self, in_c, out_c, k_s, stride, *args, **kwargs):
                ...
                self.conv3 = nn.Conv1d(out_c, out_c, 3, 1, 1)
                self.act = nn.LeakyReLU(0.2)
                if in_c != out_c:
