...
def save_states(epoch):
    states = { 'disc': disc.state_dict(), 'd_opt': optimizer_disc.state_dict(), 
            ...
            'pretrain_loss': pretrain_losses, 'pretrain_acc': pretrain_acc }

    print(len(pretrain_losses))
...
for e in range(pretrain_epochs):
    ...
    for i in range(n_its):
        ...
        # wid_ohe (256, 3606)
        encdec = ae.dec_net(ae.enc_net(wid_ohe))
        # encdec (256, 3606)
        loss = ae_loss_fn(encdec, wid)
        loss.backward()
        optimizer_ae.step()
