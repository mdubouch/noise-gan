...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.bc2 = ResBlockDown(ncom)
        self.bc3 = ResBlock(ncom*2)
        self.bc4 = ResBlockDown(ncom*2)
        self.bc5 = ResBlock(ncom*4)
        self.bc6 = ResBlockDown(ncom*4)
        self.bc7 = ResBlock(ncom*8)
    ...
    def forward(self, x_, xy_, w_): 
        ...
        x0 = torch.cat([p, pd, w, wd, g, gd], dim=1)
        x = self.bc1(self.dropout(x0))
        x = self.bc2(x)
        x = self.bc3(x)
        x = self.bc4(x)
        x = self.bc5(x)
