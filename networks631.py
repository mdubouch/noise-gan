...
class Gen(nn.Module):
    ...
    def forward(self, z, wire_to_xy):
        ...
        swg = F.softmax(w, dim=1)
        #hwg = F.one_hot(torch.argmax(w, dim=1), num_classes=n_wires).permute(0,2,1)
        wmax = w.argmax(dim=1, keepdim=True)
        hwg = torch.zeros_like(swg)
        #print(wmax.shape, hwg.shape)
        hwg.scatter_(1, wmax, 1)
        #print(hwg.shape)
        wg = hwg + swg - swg.detach()
