...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convw0 = nn.Conv1d(n_wires, 4, 1, 1, 0, bias=False)

        #self.convg0 = nn.Conv1d(geom_dim, n64, 1, 1, 0)
        
        self.convp0 = nn.Conv1d(n_features, n64, 1, 1, 0)
        self.convp1 = nn.Conv1d(n64, n64, 9, 1, 4)
        self.convp2 = nn.Conv1d(n64, n64, 5, 1, 2)

        self.convg1 = nn.Conv1d(4+geom_dim, n64, 9, 1, 4)
        self.convg2 = nn.Conv1d(n64, n64, 5, 1, 2)

        self.conv1 = nn.Conv1d(n64+n64, n128, 2, 2, 0)
        self.conv2 = nn.Conv1d(n128, n256, 2, 2, 0)
        self.conv3 = nn.Conv1d(n256, n512, 2, 2, 0)
        self.conv4 = nn.Conv1d(n256+n512, n768, 2, 2, 0)
        self.conv5 = nn.Conv1d(n768, n768, 4, 4, 0)
        self.conv6 = nn.Conv1d(n768+n768, n768*2, 4, 4, 0)
    ...
    def forward(self, x_, w_, wire_sphere): 
        ...
        p0 = self.convp0(p)
        p0 = self.act(self.convp2(self.act(self.convp1(p0))))

        w0 = self.convw0(wg)

        g0 = torch.cat([w0, xy], dim=1)
        g0 = self.act(self.convg2(self.act(self.convg1(g0))))

        x0 = torch.cat([p0, g0], dim=1)

        x1 = self.dropout(self.conv1(x0))
        #x0 = F.interpolate(x0, size=x1.shape[2], mode='nearest')
        x2 = self.conv2(self.act(x1))#self.act(torch.cat([x0, x1], dim=1)))

        x3 = self.conv3(self.act(x2))
        x2 = F.interpolate(x2, size=x3.shape[2], mode='nearest')
        x4 = self.conv4(self.act(torch.cat([x2, x3], dim=1)))

        x5 = self.conv5(self.act(x4))
        x4 = F.interpolate(x4, size=x5.shape[2], mode='nearest')
