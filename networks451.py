...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convu1 = nn.Conv1d(seq_len // 64, seq_len // 16, 1, 1, 0)
        self.lin1 = nn.Linear(8192, 6144)
        self.convu2 = nn.Conv1d(seq_len // 16, seq_len // 4, 1, 1, 0)
        self.lin2 = nn.Linear(6144, 4096)
        self.convu3 = nn.Conv1d(seq_len // 4, seq_len // 1, 1, 1, 0)
        self.lin3 = nn.Linear(4096, 4096)
        self.bn1 = nn.LayerNorm(6144)
        self.bn2 = nn.LayerNorm(4096)
        self.bn3 = nn.LayerNorm(4096)

        self.linw1 = nn.Linear(4096, n_wires)

        #self.bnp0 = nn.BatchNorm1d(n_wires)
        self.convxp = nn.ConvTranspose1d(4096, 256, 1, 1, 0)
    ...
    def forward(self, z, wire_to_xy):
        ...
        x = self.act(self.lin0(z).view(-1, self.seq_len // 64, 8192))
