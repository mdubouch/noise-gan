...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv0 = nn.Conv1d(2+1, 64, 9, 1, 4)
        self.conv1 = nn.Conv1d(64, 128, 3, 1, 1)
        self.conv2 = nn.Conv1d(128, 256, 3, 1, 1)
    ...
    def forward(self, x_): 
        ...
        r = torch.norm(w, dim=1, keepdim=True, p=2)
        #print('in', r[:,0].max().item(), r.mean().item(), r.std().item())
        w = torch.cat([w, r], dim=1)
