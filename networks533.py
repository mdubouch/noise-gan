...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convw2 = nn.Conv1d(64, n_wires, 1, 1, 0, bias=False)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convgeom = nn.Conv1d(128, 128, 3, 1, 1)
    ...
    def forward(self, x_): 
        ...
        xy = xy / xy.std(1, keepdim=True).mean(2, keepdim=True) * p.std(1, keepdim=True).mean(2, keepdim=True)

        print('P VAR %.2e' % p.var(1).mean().item())
        print('P NORM %.2e' % p.norm(dim=1).mean().item())
        print('XY VAR %.2e' % xy.var(1).mean().item())
        print('XY NORM %.2e' % xy.norm(dim=1).mean().item())
        ...
        geom = torch.cat([xy, w], dim=1)
        geom = self.act(self.convgeom(geom))
        #p = self.convp(x)
        #xy = self.convxy(xy
        x = self.dropout(torch.cat([p, geom], dim=1))

#        self.conv1.weight.register_hook(printhook)

        x0 = x
        x = self.act(self.conv1(x))
        x = self.act(x0 + self.conv2(x))
