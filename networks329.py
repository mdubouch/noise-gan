...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.linf0 = nn.Linear(512 * seq_len // 1, 512, bias=True)
        self.linf1 = nn.Linear(512, 1, bias=True)
    ...
    def forward(self, x_): 
        ...
        x = torch.cat([p, w], dim=2) #Careful dim=2 for linear (not conv)

        x = self.act(self.lin1(x))
        x = self.act(self.lin2(x))
        x = self.act(self.lin3(x))

        x = self.linf0(x.flatten(1,2))
        x = self.linf1(self.act(x))
