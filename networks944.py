...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        # Common branch
        self.convc1 = nn.Conv1d(n64*4*3 + ncat*8, 1024, 3, 1, 1)
        self.poolc = nn.MaxPool1d(2)
        self.convc2 = nn.Conv1d(1024, 1024, 3, 1, 1)
        self.convc3 = nn.Conv1d(1024, 1024, 3, 1, 1)
        self.lin0 = nn.Linear(1024, 1)
        self.dropout = nn.Dropout(0.1)
    ...
    def forward(self, x_, xy_, w_): 
        ...
        x = self.act(self.convc2(self.poolc(x)))
        x = self.act(self.convc3(self.poolc(x)))
