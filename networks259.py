...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.bnu3 = nn.Identity()#nn.InstanceNorm1d(512)

        self.convp1 = nn.ConvTranspose1d(128, 128, 1, 1, 0,  bias=True)
        self.convp2 = nn.ConvTranspose1d(128, 64, 1, 1, 0,  bias=True)
        self.convp3 = nn.ConvTranspose1d(64, n_features, 1, 1, 0,  bias=True)

        self.convw1 = nn.ConvTranspose1d(128, 128, 1, 1, 0, bias=True)
        self.convw2 = nn.ConvTranspose1d(128, 64, 1, 1, 0, bias=True)
        self.convw3 = nn.ConvTranspose1d(64, encoded_dim, 1, 1, 0, bias=True)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convp1 = nn.Conv1d(n_features, 64, 1, 1, 0, bias=True)
        self.convp2 = nn.Conv1d(64, 128, 1, 1, 0, bias=True)
        self.convp3 = nn.Conv1d(128, 128, 1, 1, 0, bias=True)
        self.convw1 = nn.Conv1d(encoded_dim, 64, 1, 1, 0, bias=True)
        self.convw2 = nn.Conv1d(64, 128, 1, 1, 0, bias=True)
