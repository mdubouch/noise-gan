...
parser.add_argument('--sequence-length', '--seq-len', type=int, default=2048)
...
train_loader, train_dataset, n_chunks = data.chunk(seq_len, batch_size=64)
...
def gradient_penalty(disc, interp_x):
    ...
    gradients_xy_pen = ((gradients_x[:,data.n_features:].flatten(1,2).norm(2, dim=1) - 1)**2).mean()
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        real_w_ohe = F.one_hot(real_w.cuda(), 
            num_classes=gu.n_wires).squeeze(1).permute(0, 2, 1).float()
        ...
        # Reshape input to discriminator so it sees two stacked samples at a time (PacGAN)
        bsize = real_p.shape[0]
        real_p = torch.cat([real_p[:bsize//2], real_p[bsize//2:]], dim=1)
        real_xy = torch.cat([real_xy[:bsize//2], real_xy[bsize//2:]], dim=1)
        real_x = torch.cat([real_p, real_xy], dim=1).requires_grad_()

        out_real = disc(real_x)

        with torch.no_grad():
            fake_p, fake_wg = sample_fake(bsize, tau)
        fake_xy = torch.tensordot(fake_wg, wire_sphere, dims=[[1], [1]]).permute(0,2,1)
        fake_p = torch.cat([fake_p[:bsize//2], fake_p[bsize//2:]], dim=1)
        fake_xy = torch.cat([fake_xy[:bsize//2], fake_xy[bsize//2:]], dim=1)
        fake_x = torch.cat([fake_p, fake_xy], dim=1).requires_grad_()
