...
optimizer_ae_pretrain = torch.optim.RMSprop(ae.parameters(),
...
print('wire_to_xy shape:', wire_to_xy.shape)
...
print('wtr', wire_to_r)
wire_to_th = torch.atan2(wire_to_xy[1], wire_to_xy[0])
print('wtth', wire_to_th)
wire_to_xy_norm = wire_to_xy / wire_to_r.max()
wire_to_r_norm = wire_to_r / wire_to_r.max()
r_losses = []
th_losses = []
for e in range(pretrain_epochs):
    ...
    batch_wxy = wire_to_xy[:,wires].permute(2, 1, 0)
    batch_r = wire_to_r_norm[wires]
    batch_th = wire_to_th[wires]
    ...
    for i in range(n_its):
        ...
        enc_r = torch.sqrt(enc[:,1]**2 + enc[:,2]**2)
        enc_th = torch.atan2(enc[:,1], enc[:,0])
        r_loss = F.mse_loss(enc_r, batch_r[i])
        th_loss = F.mse_loss(enc_th, batch_th[i])
        #ae_loss = F.mse_loss(encdec, wxy)
        #reenc = ae.enc_net(F.gumbel_softmax(encdec, dim=1, hard=True))
        #code_loss = F.mse_loss(reenc, enc)
        #code_losses.append(code_loss.item())
        loss = ae_loss + 1 * r_loss + 2 * th_loss# + 2 * code_loss
        loss.backward()
        optimizer_ae_pretrain.step()
        pretrain_losses.append(ae_loss.item())
        r_losses.append(r_loss.item())
        th_losses.append(th_loss.item())
        ...
        if e % 200 == 0 and i == 0:
            print('enc r mean', enc_th.mean().item())
            print('real r mean', wire_to_th.mean().item())
            print('enc r var', enc_th.var().item())
            print('real r var', wire_to_th.var().item())
    ...
    if e % 200 == 0:
        ...
        print('r_loss:', np.mean(r_losses[-100:]))
        print('th_loss:', np.mean(th_losses[-100:]))
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        batch_r = wire_to_r_norm[real_w]
        batch_th = wire_to_th[real_w]
        reco_loss = F.cross_entropy(real_dec_w, real_w.squeeze().cuda(), weight=wire_weights)
        enc_r = torch.norm(real_enc_w, dim=1)
        enc_th = torch.atan2(real_enc_w[:,1], real_enc_w[:,0])
        r_loss = F.mse_loss(enc_r, batch_r[i])
        th_loss = F.mse_loss(enc_th, batch_th[i])
        ae_loss = reco_loss + 10 * r_loss + 10 * th_loss
        ae_loss.backward()
