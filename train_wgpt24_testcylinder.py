...
for i in range(gu.n_layers):
    start = np.sum(gu.n_wires_per_layer[:i])
    print(theta[start] * np.pi * 2 - last)
    last = theta[start] * np.pi * 2
 
# Plot all CDC wires with clear labels (takes a while because of figure size)
#plt.figure(figsize=(48,48))
#ax = plt.gca()
#ax.scatter(gu.wire_x[np.arange(gu.n_wires)], gu.wire_y[np.arange(gu.n_wires)],
#        s=1)
#for i in range(gu.n_wires):
#        ax.annotate(str(i), (gu.wire_x[i], gu.wire_y[i]))
#plt.savefig(output_dir+'wire_labels.png', dpi=120)
#plt.close()
#print(gu.cum_n_wires)
