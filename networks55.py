...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len):
        ...
        self.convw3 = nn.Conv1d(ngf*2, ngf, 513, 1, 256)
        self.convw = nn.Conv1d(ngf, n_wires, 1, 1, 0)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len):
        ...
        self.convw = nn.Conv1d(2, ndf*1, 1, 1, 0)
        self.resw0 = ResBlock(ndf, ndf*2, 513, 1, 256, padding_mode='zeros')
        self.resw1 = ResBlock(ndf*2, ndf*2, 17, 1, 8, padding_mode='zeros')

        self.convp = nn.Conv1d(n_features, ndf, 1, 1, 0)
        self.resp0 = ResBlock(ndf, ndf*2, 33, 1, 16, padding_mode='zeros')
        self.resp1 = ResBlock(ndf*2, ndf*2, 17, 1, 8, padding_mode='zeros')
        

        self.res1 = ResBlock(ndf*4, ndf*4, 33, 1, 16, padding_mode='zeros')
        self.res2 = ResBlock(ndf*4, ndf*6, 17, 1, 8, padding_mode='zeros')


        self.convd1 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 4, 2, 1, padding_mode='zeros')) # // 2
        self.conv1 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 9, 1, 4))

        self.convd2 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 4, 2, 1, padding_mode='zeros')) # // 4
        self.conv2 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 9, 1, 4))

        self.convd3 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 4, 2, 1, padding_mode='zeros')) # // 8
        self.conv3 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 9, 1, 4))
        self.convd4 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 4, 2, 1, padding_mode='zeros')) # // 16
        self.conv4 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 9, 1, 4))
        self.convd5 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 4, 2, 1, padding_mode='zeros')) # // 32
        self.conv5 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 9, 1, 4))
        self.convd6 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 4, 2, 1, padding_mode='zeros')) # // 64
        self.conv6 = nn.utils.spectral_norm(nn.Conv1d(ndf*6, ndf*6, 9, 1, 4))

        self.lin0 = nn.Linear(ndf*6 * seq_len // 64, 1, bias=True)
    ...
    def forward(self, p_, w_): # p_ shape is (batch, features, seq_len), w_ shape is one-hot encoded wire (batch, 4986, seq_len)
        ...
        x0 = x
        x = self.act(self.convd1(x0))
        x = self.act(self.conv1(x) + F.interpolate(x0, scale_factor=0.5, mode='linear'))

        x0 = x
        x = self.act(self.convd2(x0))
        x = self.act(self.conv2(x) + F.interpolate(x0, scale_factor=0.5, mode='linear'))

        x0 = x
        x = self.act(self.convd3(x0))
        x = self.act(self.conv3(x) + F.interpolate(x0, scale_factor=0.5, mode='linear'))

        x0 = x
        x = self.act(self.convd4(x0))
        x = self.act(self.conv4(x) + F.interpolate(x0, scale_factor=0.5, mode='linear'))

        x0 = x
        x = self.act(self.convd5(x0))
        x = self.act(self.conv5(x) + F.interpolate(x0, scale_factor=0.5, mode='linear'))

        x0 = x
        x = self.act(self.convd6(x0))
        x = self.act(self.conv6(x) + F.interpolate(x0, scale_factor=0.5, mode='linear'))
