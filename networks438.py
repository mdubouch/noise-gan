...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.lin0 = nn.Linear(latent_dims, seq_len//8*4096, bias=True)
        ...
        self.conv1 = nn.ConvTranspose1d(4096, 3900, 2, 2, 0)
        self.conv2 = nn.ConvTranspose1d(3900, 3800, 2, 2, 0)
        self.conv3 = nn.ConvTranspose1d(3800, 3700, 2, 2, 0)
        self.bn1 = nn.BatchNorm1d(3900)
        self.bn2 = nn.BatchNorm1d(3800)
        self.bn3 = nn.InstanceNorm1d(3700)

        self.convw1 = nn.ConvTranspose1d(3700, n_wires, 1, 1, 0)

        #self.bnp0 = nn.BatchNorm1d(n_wires)
        self.convxp = nn.ConvTranspose1d(3700, 256, 1, 1, 0)
    ...
    def forward(self, z, wire_to_xy):
        ...
        x = self.act(self.lin0(z).view(-1, 4096, self.seq_len // 8))
