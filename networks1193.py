...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim, n_wires):
        ...
        self.convp = nn.ConvTranspose1d(64, 3, 1, 1, 0)
        self.convw = nn.ConvTranspose1d(64, 16, 1, 1, 0)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim, n_wires):
        ...
        self.convp = nn.Conv1d(3, 32, 1, 1, 0)
        self.convw = nn.Conv1d(16, 32, 1, 1, 0)
    ...
    def forward(self, x_):
        ...
        p = self.act(self.normp(self.convp(x_[:,:3])))
        w = self.act(self.normw(self.convw(x_[:,3:])))
