...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.lin0 = nn.Linear(latent_dims, seq_len//256*1024, bias=True)
        ...
        class GBlock(nn.Module):
            def __init__(self, in_channels, out_channels):
                ...
                self.bnp = nn.InstanceNorm1d(out_channels)
            ...
            def forward(self, x):
                y0 = self.bnp(F.interpolate(self.convp(x), scale_factor=2))
                y = self.act(self.bnu(self.convu(x)))
                y = self.act(self.bn1(self.conv1(y)))
                y = self.act(y0 + self.bn2(self.conv2(y)))
                return y
        self.gb1 = GBlock(1024, 1024)
        self.gb2 = GBlock(1024, 1024)
        self.gb3 = GBlock(1024, 1024)
        self.gb4 = GBlock(1024, 1024)
        self.gb5 = GBlock(1024, 512)
    ...
    def forward(self, z, embed_space_noise=0.0, tau=1.0):
        ...
        x = self.act(self.lin0(z).view(-1, 1024, self.seq_len // 256))
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        class DBlock(nn.Module):
            ...
            def forward(self, x):
                y0 = F.interpolate(self.convp(x), scale_factor=0.5)
        ...
        self.lin0 = nn.Linear(1024 * seq_len // 256, 128, bias=True)
