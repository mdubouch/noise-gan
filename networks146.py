...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.conv1 = nn.Conv1d(ngf*6, ngf*6, 3, 1, 1)
        self.conv2 = nn.Conv1d(ngf*6, ngf*6, 3, 1, 1)
        self.conv3 = nn.Conv1d(ngf*6, ngf*6, 3, 1, 1)
        self.conv4 = nn.Conv1d(ngf*6, ngf*4, 3, 1, 1)
        ...
        self.conv7 = nn.Conv1d(ngf*4, ngf*3, 3, 1, 1)
        self.bn7 = nn.InstanceNorm1d(ngf*3)
        self.conv8 = nn.ConvTranspose1d(ngf*3, ngf*3, 3, 1, 1)
        self.bn8 = nn.InstanceNorm1d(ngf*3)

        self.convp1 = nn.ConvTranspose1d(ngf*3, ngf, 3, 1, 1)
        self.convp2 = nn.ConvTranspose1d(ngf*1, ngf, 3, 1, 1)
        self.convp3 = nn.ConvTranspose1d(ngf*1, ngf, 3, 1, 1)
        self.convp4 = nn.ConvTranspose1d(ngf, n_features, 1, 1, 0)

        self.convw1 = nn.ConvTranspose1d(ngf*3, ngf*3, 3, 1, 1)
        self.convw2 = nn.ConvTranspose1d(ngf*3, ngf*3, 3, 1, 1)
        self.convw3 = nn.ConvTranspose1d(ngf*3, ngf*3, 3, 1, 1)
        self.convw4 = nn.ConvTranspose1d(ngf*3, ngf*3, 3, 1, 1)
        self.convw5 = nn.ConvTranspose1d(ngf*3, ngf*3, 3, 1, 1)
        self.convw6 = nn.ConvTranspose1d(ngf*3, encoded_dim, 1, 1, 0)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convp1 = nn.Conv1d(n_features, ndf*1, 3, 1, 1)
        self.convp2 = nn.Conv1d(ndf, ndf, 3, 1, 1)
        self.convp3 = nn.Conv1d(ndf, ndf, 3, 1, 1)
        #self.convw_diff1 = nn.Conv1d(encoded_dim, ndf*2, 1, 1, 0)
        self.convw1 = nn.Conv1d(encoded_dim, ndf*3, 3, 1, 1)
        self.convw2 = nn.Conv1d(ndf*3, ndf*3, 3, 1, 1)
        self.convw3 = nn.Conv1d(ndf*3, ndf*3, 3, 1, 1)

        self.conv1 = nn.Conv1d(ndf*4, ndf*8, 4, 2, 1)
        self.conv2 = nn.Conv1d(ndf*8, ndf*8, 4, 2, 1)
        self.conv3 = nn.Conv1d(ndf*8, ndf*8, 4, 2, 1)
        self.conv4 = nn.Conv1d(ndf*8, ndf*8, 4, 2, 1)
        self.conv5 = nn.Conv1d(ndf*8, ndf*8, 4, 2, 1)
        self.conv6 = nn.Conv1d(ndf*8, ndf*8, 4, 2, 1)

        self.lin0 = nn.Linear(ndf*8 * seq_len // 64, 1, bias=True)
