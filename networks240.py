...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.lin0 = nn.Linear(latent_dims, seq_len//8*ngf*1, bias=True)

        self.convu1 = nn.ConvTranspose1d(ngf*1, ngf*1, 4, 2, 1)
        self.bnu1 = nn.InstanceNorm1d(ngf*1)
        self.convu2 = nn.ConvTranspose1d(ngf*1, ngf*1, 4, 2, 1)
        self.bnu2 = nn.InstanceNorm1d(ngf*1)
        self.convu3 = nn.ConvTranspose1d(ngf*1, ngf*1, 4, 2, 1)
        self.bnu3 = nn.InstanceNorm1d(ngf*1)

        self.conv0 = nn.ConvTranspose1d(ngf*1, ngf*1, 1, 1, 0)
        ...
        self.convp1 = nn.ConvTranspose1d(ngf, ngf, 1, 1, 0)
        self.convp2 = nn.ConvTranspose1d(ngf, n_features, 1, 1, 0)
        self.convw1 = nn.ConvTranspose1d(ngf, ngf, 1, 1, 0)
        self.convw2 = nn.ConvTranspose1d(ngf, encoded_dim, 1, 1, 0)

        self.out = nn.Tanh()
        
    def forward(self, z, embed_space_noise=0.0, tau=1.0):
        # z: random point in latent space
        x = self.act(self.lin0(z).view(-1, self.ngf*1, self.seq_len // 8))
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convp2 = nn.Conv1d(ndf, ndf*4, 1, 1, 0)
        self.convw1 = nn.Conv1d(encoded_dim, ndf*1, 1, 1, 0)
        self.convw2 = nn.Conv1d(ndf, ndf*4, 1, 1, 0)
