...
# Initialize networks
gen = networks.Gen(ngf=ngf, latent_dims=latent_dims, seq_len=seq_len, embedding_dim=embedding_dim,
    n_wires=gu.n_wires).to(device)
disc = networks.Disc(ndf=ndf, seq_len=seq_len, embedding_dim=embedding_dim,
    n_wires=gu.n_wires).to(device)
print('Gen summary:')
...
n_features=3
gen.eval()
def sample_fake(batch_size=None, z=None):
    if z is None:
        z = torch.randn((batch_size, latent_dims)).to(device)
    p, w = gen(z)

    dec_w =  embed.lin(w.permute(0,2,1)).permute(0,2,1)
...
for i in range(n):
    with torch.no_grad():
        ...
        latent_var = torch.randn((n_seqs, latent_dims)).to(device)
        p, w = sample_fake(z=latent_var)
