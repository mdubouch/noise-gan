...
def sample_real(batch_size):
    ...
    return p, one_hot_w
def sample_fake(batch_size, tau, embed_noise):
    noise = to_device(torch.randn((batch_size, latent_dims), requires_grad=True))
    sample = gen(noise, embed_noise, tau)
    return sample

_p, _w = sample_real(2)
print(_p.shape, _w.shape)

__f = sample_fake(2, 1.0, 0.1)
...
for e in range(n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        fake_p, fake_w = sample_fake(real_p.shape[0], tau, noise_level)
        ...
        if (i % n_critic == 0):
            ...
            fake_hits = sample_fake(real_p.shape[0], tau, noise_level)
            fake_p = fake_hits[0]
            fake_w = fake_hits[1]
