...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim, n_wires):
        ...
        self.lin2 = nn.Linear(64, seq_len*16)

        self.convw1 = nn.ConvTranspose1d(16, 16, 3, 1, 1)
        self.convp1 = nn.ConvTranspose1d(16, 16, 3, 1, 1)

        self.convp = nn.ConvTranspose1d(16, n_features, 1, 1, 0, bias=True)
        self.convw = nn.ConvTranspose1d(16, n_wires, 1, 1, 0, bias=True)
        self.out = nn.Tanh()
    ...
    def forward(self, z):
        ...
        x = self.act(self.lin2(x)).reshape(-1, 16, self.seq_len)
