...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len):
        ...
        self.convw3 = nn.Conv1d(ngf*2, n_wires, 33, 1, 16)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len):
        ...
        self.resw0 = ResBlock(2, ndf*2, 33, 1, 16, padding_mode='zeros')
    ...
    def forward(self, p_, w_): # p_ shape is (batch, features, seq_len), w_ shape is one-hot encoded wire (batch, 4986, seq_len)
        ...
        x = self.act(self.conv1(x) + F.interpolate(x0, scale_factor=0.5))

        x0 = x
        x = self.act(self.convd2(x0))
        x = self.act(self.conv2(x) + F.interpolate(x0, scale_factor=0.5))

        x0 = x
        x = self.act(self.convd3(x0))
        x = self.act(self.conv3(x) + F.interpolate(x0, scale_factor=0.5))

        x0 = x
        x = self.act(self.convd4(x0))
        x = self.act(self.conv4(x) + F.interpolate(x0, scale_factor=0.5))

        x0 = x
        x = self.act(self.convd5(x0))
        x = self.act(self.conv5(x) + F.interpolate(x0, scale_factor=0.5))

        x0 = x
        x = self.act(self.convd6(x0))
        x = self.act(self.conv6(x) + F.interpolate(x0, scale_factor=0.5))
