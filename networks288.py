...
class Disc(nn.Module):
    ...
    def forward(self, x_): 
        ...
        p = self.dbp3(p)
        p = self.act(self.resp(F.interpolate(p0, scale_factor=1/8, mode='linear')) + p)

        w0 = self.act(self.convw(w))
        w = self.act(self.dbw1(w0))
        w = self.act(self.dbw2(w))
        w = self.dbw3(w)
        p = self.act(self.resw(F.interpolate(w0, scale_factor=1/8, mode='linear')) + w)
        ...
        x = self.db3(x)
        x = self.act(F.interpolate(x0, scale_factor=1/8, mode='linear') + x)
