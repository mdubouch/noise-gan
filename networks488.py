...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.conv1 = nn.Conv1d(256, 192, 5, 1, 2)
        self.bn1 = nn.BatchNorm1d(192)
        self.conv2 = nn.Conv1d(192, 128, 9, 1, 4)

        self.convw = nn.Conv1d(128, n_wires, 1, 1, 0, bias=False)
        self.convp1 = nn.Conv1d(128, 64, 17, 1, 8)
        self.bnp = nn.InstanceNorm1d(64)
    ...
    def forward(self, z, wire_to_xy):
        ...
        w = self.convw(x)
        p = torch.sigmoid(self.bnp(self.convp1(x)))
        p = self.convp2(p)
