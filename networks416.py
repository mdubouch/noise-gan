...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convw1 = nn.ConvTranspose1d(256, 192, 1, 1, 0)
        self.bnw1 = nn.InstanceNorm1d(192)
        self.convw2 = nn.ConvTranspose1d(192, 128, 1, 1, 0)
        self.bnw2 = nn.InstanceNorm1d(128)
        self.convw3 = nn.ConvTranspose1d(128, n_wires, 1, 1, 0)

        #self.bnp0 = nn.BatchNorm1d(n_wires)
        self.convwp = nn.ConvTranspose1d(128, 64, 1, 1, 0)
    ...
    def forward(self, z, wire_to_xy):
        ...
        p = self.act(self.bnp1(self.convwp(wf)))
        p = self.convp2(p)
