...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.conv1 = nn.ConvTranspose1d(ngf*8, ngf*8, 32, 8, 12)
        self.bn1 = nn.InstanceNorm1d(ngf*8)

        self.conv7 = nn.Conv1d(ngf*8, ngf*8, 3, 1, 1)
        self.bn7 = nn.InstanceNorm1d(ngf*8)

        self.convp1 = nn.ConvTranspose1d(ngf*8, ngf*2, 65, 1, 32)
        self.convp2 = nn.ConvTranspose1d(ngf*2, ngf*1, 33, 1, 16)
        self.convp3 = nn.ConvTranspose1d(ngf*1, n_features, 17, 1, 8)

        self.convw1 = nn.ConvTranspose1d(ngf*8, ngf*4, 65, 1, 32)
        self.convw2 = nn.ConvTranspose1d(ngf*4, ngf*1, 33, 1, 16)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convp4 = nn.Conv1d(ndf*4, ndf*4, 3, 1, 1)
        ...
        self.convw4 = nn.Conv1d(ndf*8, ndf*8, 3, 1, 1)

        self.convxy1 = nn.Conv1d(2, ndf*2, 33, 8, 16)
        self.convxy2 = nn.Conv1d(ndf*2, ndf*2, 3, 1, 1)
        self.convxy3 = nn.Conv1d(ndf*2, ndf*4, 33, 8, 16)
        self.convxy4 = nn.Conv1d(ndf*4, ndf*4, 3, 1, 1)
        
        self.convdist1 = nn.Conv1d(1, ndf*2, 33, 8, 16)
        self.convdist2 = nn.Conv1d(ndf*2, ndf*2, 3, 1, 1)
        self.convdist3 = nn.Conv1d(ndf*2, ndf*2, 33, 8, 16)
        self.convdist4 = nn.Conv1d(ndf*2, ndf*2, 3, 1, 1)

        self.conv1 = nn.Conv1d(ndf*18, ndf*12, 3, 1, 1)
        self.conv2 = nn.Conv1d(ndf*12, ndf*8, 3, 1, 1)
