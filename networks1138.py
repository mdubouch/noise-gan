...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim, n_wires):
        ...
        self.lin0 = nn.Linear(latent_dims, 512*4)
    ...
    def forward(self, z):
        ...
        x = self.norm0(self.act(self.lin0(z)).view(-1, 512, 4))
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim, n_wires):
        ...
        self.s1 = Simple(1+n_features+nproj+geom_dim, 64, 3, 1, 1)
        self.s2 = Simple(1+n_features+nproj+geom_dim, 64, 3, 1, 3, dilation=3)
        self.s3 = Simple(1+n_features+nproj+geom_dim, 64, 3, 1, 9, dilation=9)
        self.s4 = Simple(1+n_features+nproj+geom_dim, 64, 3, 1, 27, dilation=27)
        self.s5 = Simple(1+n_features+nproj+geom_dim, 64, 3, 1, 81, dilation=81)
        self.s6 = Simple(1+n_features+nproj+geom_dim, 64, 3, 1, 243, dilation=243)

        self.c1 = Simple(64*6, 64*8, 3, 1, 1)
        self.c2 = Simple(64*8, 64*8, 1, 1, 0)

        self.lin0 = nn.Linear(64*8, 1)
