...
fw = torch.multinomial(w.permute(0,2,1).flatten(0,1), 1, replacement=True).squeeze().detach().cpu()
plt.figure()
...
#plt.figure()
#plt.hist(inv_p[:,3].cpu(), bins=50)
#plt.savefig(output_dir+'gen_progx.png')
#plt.close()
#plt.figure()
#plt.hist(inv_p[:,4].cpu(), bins=50)
#plt.savefig(output_dir+'gen_progy.png')
#plt.close()
#plt.figure()
#plt.hist(torch.atan2(inv_p[:,4], inv_p[:,3]).cpu(), bins=50)
#plt.savefig(output_dir+'gen_progtheta.png')
#plt.close()
...
for i in range(n_its):
    gen.eval()
    fake_p, fake_w = sample_fake(n_seqs)
...
print(w.shape)
#fw = torch.argmax(w, dim=1).flatten().detach().cpu()
fw = torch.multinomial(w.squeeze().T, 1, replacement=True).squeeze().detach().cpu()
print(fw.shape)
print(fw)
inv_p = data.inv_preprocess(p.permute(0, 2, 1).flatten(0, 1))
...
p, w = sample_fake(8)#data.wire.size // seq_len, tau)
#fw = torch.argmax(w, dim=1).flatten().detach().cpu()
fw = torch.multinomial(w.permute(0,2,1).flatten(0,1), 1, replacement=True).squeeze().detach().cpu()
fx = gu.wire_x[fw]
fy = gu.wire_y[fw]
...
for i in range(n_samples):
    for j in range(n_samples):
        ...
        w = torch.multinomial(w.permute(0,2,1).flatten(0,1), 1, replacement=True).squeeze().detach().cpu()
        #plt.title('G4')
        ax = plt.subplot(gs1[i*4 + j])
        gu.draw_cdc(ax)
        ax.set_aspect(1)
...
for i in range(n_seq):
    fake_p, fake_w = sample_fake(1)
    real_p, real_w = sample_real(1)

    #fw = torch.argmax(fake_w, dim=1).flatten().detach().cpu()
    fw = torch.multinomial(fake_w.permute(0,2,1).flatten(0,1), 1, replacement=True).squeeze().detach().cpu()
    rw = real_w.squeeze()
...
for i in range(n):
    with torch.no_grad():
        ...
        fake_wire[i*seq_len*n_seqs:(i+1)*seq_len*n_seqs] = torch.multinomial(w.permute(0,2,1).flatten(0,1), 1, replacement=True).squeeze().detach().cpu()
        inv_p = data.inv_preprocess(p.permute(0,2,1).flatten(0,1)) 
