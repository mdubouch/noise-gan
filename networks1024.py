...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv1 = nn.Conv1d(nproj+geom_dim+n_features, n64//2, 7, 1, 3, padding_mode='circular')
        self.convd = nn.Conv1d(nproj+geom_dim+n_features, n64//2, 7, 1, 3, padding_mode='circular')
        self.conv2 = nn.Conv1d(n64, n64*2, 3, 2, 1, padding_mode='circular')
        self.conv3 = nn.Conv1d(n64*2, n64*4, 3, 1, 1, padding_mode='circular')
    ...
    def forward(self, x_, xy_, w_): 
        ...
        xydiff = (xy - self.padleft(xy[:,:,:-1]))**2
        print(xydiff.mean().item(), xydiff.std().item())

        w0 = self.convw0(wg)

        x = torch.cat([w0, p, xy], dim=1)

        d = self.act(self.convd(x - self.padleft(x[:,:,:-1])))
