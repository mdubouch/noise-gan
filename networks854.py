...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convw3 = nn.Conv1d(n64, n64, 1, 1, 0)

        self.convg0 = nn.Conv1d(geom_dim, 4, 1, 1, 0, bias=False)
        self.convg1 = nn.Conv1d(4, 8, 1, 1, 0)
        self.convg2 = nn.Conv1d(8, 16, 1, 1, 0)
        self.convg3 = nn.Conv1d(16, n64, 1, 1, 0)
        
        self.convp0 = nn.Conv1d(n_features, n64, 1, 1, 0)

        self.conv1 = nn.Conv1d(n64+n64+n64, n128, 2, 2, 0)
        self.conv2 = nn.Conv1d(n128, n256, 2, 2, 0)
        self.conv3 = nn.Conv1d(n256, n512, 2, 2, 0)
        self.conv4 = nn.Conv1d(n512, n512, 4, 4, 0)
        self.conv5 = nn.Conv1d(n512, n512, 4, 4, 0)
    ...
    def forward(self, x_, w_, wire_sphere): 
        ...
        p0 = self.convp0(p)
        w0 = self.convw3(self.act(self.convw2(self.act(self.convw1(self.convw0(wg))))))
        g0 = self.convg3(self.act(self.convg2(self.act(self.convg1(self.convg0(xy))))))
        ...
        x2 = self.dropout(self.conv2(self.act(x1)))#self.act(torch.cat([x0, x1], dim=1)))
