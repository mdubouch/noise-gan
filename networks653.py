...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv0 = nn.utils.spectral_norm(nn.Conv1d(n_features, n192, 64, 2, 0))

        n256 = n64 * 4
        self.conv1 = nn.utils.spectral_norm(nn.Conv1d(n192, n256, 32, 2, 0))

        n512 = n256 * 2
        self.conv2 = nn.utils.spectral_norm(nn.Conv1d(n256, n512, 16, 2, 0))
        self.conv3 = nn.utils.spectral_norm(nn.Conv1d(n512, n512, 8, 2, 0))
        self.conv4 = nn.utils.spectral_norm(nn.Conv1d(n256+n512, n512, 4, 4, 0))
        self.conv5 = nn.utils.spectral_norm(nn.Conv1d(n512, n512, 2, 4, 0))
    ...
    def forward(self, x_): 
        ...
        x = self.act(torch.cat([x, x1], dim=1))
        #x = F.max_pool1d(x, 2)
