...
# Initialize networks
gen = networks.Gen(ngf=ngf, latent_dims=latent_dims, seq_len=seq_len, encoded_dim=encoded_dim).to(device)
...
for i in range(len(save_files)):
    ...
    ax = plt.gca()
    only_walls(ax)
    plt.scatter(gu.wire_x[w], gu.wire_y[w], s=1+p[:,0]*1e3, c=p[:,2], cmap='inferno',
