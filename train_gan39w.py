...
for i in range(gu.n_layers):
    ...
    wire_sphere[0,start:end] = torch.from_numpy(np.sqrt(x**2+y**2))
...
def gradient_penalty(disc, interp_x):
    ...
    #print('GRADIENTS SHAPE', gradients_x.shape)
    #print('GRADIENTS P SHAPE', gradients_x[:,:data.n_features].shape)
    #print('GRADIENTS XY SHAPE', gradients_x[:,data.n_features:data.n_features+2].shape)
    #print('GRADIENTS W SHAPE', gradients_x[:,data.n_features+2:].shape)
    gradients_p_pen = ((gradients_x[:,:data.n_features].flatten(1,2).norm(2, dim=1) - 1)**2).mean()
    gradients_xy_pen = ((gradients_x[:,data.n_features:data.n_features+geom_dim].flatten(1,2).norm(2, dim=1) - 1)**2).mean()
