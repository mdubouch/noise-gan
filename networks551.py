...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        n192 = n64 * 3
        self.conv1 = nn.utils.spectral_norm(nn.Conv1d(n192, n192, 3, 1, 1))
    ...
    def forward(self, x_): 
        ...
        p = p + torch.randn_like(p) * 0.01
        xy = xy + torch.randn_like(xy) * 0.01
        wg = wg + torch.randn_like(wg) * 0.01
        ...
        #p = self.convp(x)
        #xy = self.convxy(xy
        x = torch.cat([p / 3, xy / 2, w / 3606], dim=1)
