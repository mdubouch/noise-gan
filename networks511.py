...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.bnu1 = nn.BatchNorm1d(384)
        self.convu2 = nn.ConvTranspose1d(384, 256, 4, 4, 0)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        class DBlock(nn.Module):
            ...
            def forward(self, x):
                y = self.act(self.bn(self.convd(x)))
                return y

        self.convp1 = nn.Conv1d(n_features, 32, 3, 1, 1)
        self.bnp1 = nn.Identity()#nn.InstanceNorm1d(32)
        self.convp2 = nn.Conv1d(32, 64, 3, 1, 1)
        self.bnp2 = nn.Identity()#nn.InstanceNorm1d(64)
        self.convxy1 = nn.Conv1d(2, 32, 3, 1, 1)
        self.bnxy1 = nn.Identity()#nn.InstanceNorm1d(32)
        self.convxy2 = nn.Conv1d(32, 64, 3, 1, 1)
        self.bnxy2 = nn.Identity()#nn.InstanceNorm1d(64)
        self.conv1 = nn.Conv1d(128, 128, 17, 1, 8)
    ...
    def forward(self, x_): 
        ...
        #w = self.convw(wg)
        xy = self.act(self.bnxy1(self.convxy1(xy)))
        xy = self.act(self.bnxy2(self.convxy2(xy)))
