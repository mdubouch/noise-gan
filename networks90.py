...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len):
        ...
        self.tcn2 = TCNBlockTranspose(ngf*1, ngf*1, 512)

        self.convw3 = nn.Conv1d(ngf*1, ngf*1, 1, 1, 0)
        self.convw4 = nn.Conv1d(ngf*1, encoded_dim, 1, 1, 0)
    ...
    def forward(self, z, embed_space_noise=0.0, tau=1.0):
        ...
        w = self.tcn1(w)
        w = self.tcn2(w)
        w = self.act(nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1]) + self.convw3(w))
        w = self.convw4(w)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len):
        ...
        self.convw3 = nn.Conv1d(encoded_dim, ndf*1, 1, 1, 0)
        self.convw4 = nn.Conv1d(ndf*1, ndf*1, 1, 1, 0)
        self.tcn1 = TCNBlock(ndf*1, ndf*1, 512)
    ...
    def forward(self, x_): 
        ...
        w = self.act(nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1]) + self.convw4(w)) 
        w = self.tcn1(w)
        w = self.tcn2(w)
        #w = self.resw2(w)
        #w = self.resw3(w)
