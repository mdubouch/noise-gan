...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convu1 = nn.Conv1d(seq_len // 64, seq_len // 1, 1, 1, 0)
        self.conv0 = nn.Conv1d(512, 512, 1, 1, 0)
        self.bn0 = nn.InstanceNorm1d(512)

        self.conv1 = nn.Conv1d(512, 256+n_features, 1, 1, 0)
    ...
    def forward(self, z, wire_to_xy):
        ...
        x = self.act(self.lin0(z).view(-1, self.seq_len // 64, 512))

        x = self.act(self.convu1(x))
        x = self.act(self.bn0(self.conv0(x.permute(0,2,1))))
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv1 = nn.Conv1d(256+n_features, 512, 1, 1, 0)
        self.conv2 = nn.Conv1d(512, 512, 9, 1, 4)
