...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.lin0 = nn.Linear(latent_dims, seq_len//64*ngf*8, bias=True)
        ...
        self.convu1 = nn.ConvTranspose1d(ngf*8, ngf*8, 4, 2, 1)
        self.convu2 = nn.ConvTranspose1d(ngf*8, ngf*8, 4, 2, 1)
        self.convu3 = nn.ConvTranspose1d(ngf*8, ngf*8, 4, 2, 1)
        self.convu4 = nn.ConvTranspose1d(ngf*8, ngf*4, 4, 2, 1) # * 2
        self.convu5 = nn.ConvTranspose1d(ngf*4, ngf*2, 4, 2, 1) # * 4
        self.convu6 = nn.ConvTranspose1d(ngf*2, ngf*1, 4, 2, 1) # * 8
        self.bnu1 = nn.InstanceNorm1d(ngf*8)
        self.bnu2 = nn.InstanceNorm1d(ngf*8)
        self.bnu3 = nn.InstanceNorm1d(ngf*8)
        self.bnu4 = nn.InstanceNorm1d(ngf*4)
        self.bnu5 = nn.InstanceNorm1d(ngf*2)
        self.bnu6 = nn.InstanceNorm1d(ngf*1)

        self.convw1 = nn.Conv1d(ngf, ngf, 9, 1, 4)
        self.convw = nn.Conv1d(ngf, encoded_dim, 1, 1, 0)

        self.convp1 = nn.Conv1d(ngf, ngf, 9, 1, 4)
    ...
    def forward(self, z, embed_space_noise=0.0, tau=1.0):
        ...
        x = self.act(self.lin0(z).view(-1, self.ngf*8, self.seq_len // 64))
