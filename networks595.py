...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convx1 = nn.utils.spectral_norm(nn.Conv1d(emb_dim*4, emb_dim*4, 3, 1, 1))
        self.convx2 = nn.utils.spectral_norm(nn.Conv1d(emb_dim*4, emb_dim*4, 3, 1, 1))
    ...
    def forward(self, x_): 
        ...
        x = self.memb(torch.cat([p, w], dim=1))

        x0 = self.convx1(x)
        x = self.act(x0)
        x = self.act(self.convx2(x))
