...
for e in range(pretrain_epochs):
    ...
    wires = wires.view(bsize, 1)
    n_its = gu.n_wires // bsize + 1
    code_losses = []
    for i in range(n_its):
        optimizer_ae_pretrain.zero_grad()
        # wid (256,)
        wid_ohe = F.one_hot(wires, num_classes=gu.n_wires).squeeze(1).float().requires_grad_(True)        # wid_ohe (3606, 3606)

        enc = ae.enc_net(wid_ohe)
        # enc (bsize, enc_dim, 1)

        encdec = ae.dec_net(enc)
        ae_loss = F.cross_entropy(encdec, wires.squeeze(1), weight=wire_weights)
        ...
        hit = (wires.squeeze() == choice).sum().float()
        acc = hit / bsize
