...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convp2 = nn.ConvTranspose1d(ngf, n_features, 1, 1, 0)
        ...
        self.convw4_ = nn.ConvTranspose1d(ngf*3, encoded_dim, 1, 1, 0)
    ...
    def forward(self, z, embed_space_noise=0.0, tau=1.0):
        ...
        w_shifted = nn.ConstantPad1d((1, 0), 0.0)(x[:,:,:-1])
        w = self.act(w_shifted + self.convw1(w_shifted))
        w_shifted = nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1])
        w = self.act(w_shifted + self.convw2(w_shifted))
        w_shifted = nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1])
        w = self.act(w_shifted + self.convw3(w_shifted))
        w_shifted = nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1])
        w = self.convw4_(w_shifted) + self.convw4(w)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convw1 = nn.Conv1d(encoded_dim, ndf*2, 65, 1, 32)
        self.convw2 = nn.Conv1d(ndf*2, ndf*3, 65, 1, 32)
        self.convw3 = nn.Conv1d(ndf*3, ndf*4, 65, 1, 32)
        self.convw_diff1 = nn.Conv1d(encoded_dim, ndf*2, 1, 1, 0)
        self.convw_diff2 = nn.Conv1d(ndf*2, ndf*3, 1, 1, 0)
        self.convw_diff3 = nn.Conv1d(ndf*3, ndf*4, 1, 1, 0)
        self.conv1 = nn.Conv1d(ndf*5, ndf*8, 1, 1, 0)
        self.conv2 = nn.Conv1d(ndf*8, ndf*8, 1, 1, 0)
        self.conv3 = nn.Conv1d(ndf*8, ndf*8, 1, 1, 0)

        self.lin0 = nn.Linear(ndf*8 * seq_len, 1, bias=True)
    ...
    def forward(self, x_): 
        ...
        w = self.act(self.convw1(w) + w_diff)

        w_diff = self.act(self.convw_diff2(w - nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1])))
        w = self.act(self.convw2(w) + w_diff)

        w_diff = self.act(self.convw_diff3(w - nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1])))
        w = self.act(self.convw3(w) + w_diff)
