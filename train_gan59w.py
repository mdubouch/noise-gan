...
def gradient_penalty(disc, interp_p, interp_xy, interp_w):
    ...
    gradient_pen = torch.mean(gradients_p_pen + gradients_xy_pen)
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        real_xy = torch.tensordot(real_w_ohe, wire_sphere+torch.randn_like(wire_sphere)*0.01, 
                dims=[[1], [1]]).permute(0,2,1)
        ...
        fake_xy = torch.tensordot(fake_wg, wire_sphere+torch.randn_like(wire_sphere)*0.01, dims=[[1], [1]]).permute(0,2,1)
        ...
        interpolates_xy = torch.tensordot(interpolates_w, 
                wire_sphere+torch.randn_like(wire_sphere)*0.01, dims=[[1], [1]]).permute(0,2,1)
        ...
        if (critic_count % n_critic == 0):
            ...
            fake_xy = torch.tensordot(fake_wg, 
                    wire_sphere+torch.randn_like(wire_sphere)*0.01, dims=[[1], [1]]).permute(0,2,1)
            #fake_xy = fake_xy + torch.randn_like(fake_xy) * 1.1
            #print('fk',fake_xy[0,:,0])
            #fake_x = torch.cat([fake_p, fake_wg], dim=1)
