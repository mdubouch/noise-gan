...
class Disc(nn.Module):
    ...
    def forward(self, x_): 
        ...
        x1 = self.conv1(self.dropout(self.act(x0)))
        x2 = self.conv2(self.dropout(self.act(x1)))
        x3 = self.conv3(self.dropout(self.act(x2)))

        x = self.act(self.conv4(self.dropout(self.act(x3))))
        x = self.act(self.conv5(self.dropout(x)))

        x = self.lin0(self.dropout(x.mean(2)))
