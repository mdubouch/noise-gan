...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convw3 = nn.Conv1d(n64, geom_dim, 1, 1, 0, bias=False)
    ...
    def forward(self, z, wire_to_xy):
        ...
        #plt.close()
        #wg = F.gumbel_softmax(1/wdist, dim=2, hard=True, tau=2/3).permute(0,2,1)
        wg = F.softmin(wdist, dim=2).permute(0,2,1)
        #print(wg.shape)
        #exit(1)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.xyemb = nn.utils.spectral_norm(nn.Conv1d(geom_dim, emb_dim, 1, 1, 0, bias=False))
        ...
        self.conv0 = nn.utils.spectral_norm(nn.Conv1d(emb_dim*1, n192, 3, 1, 1))
    ...
    def forward(self, x_): 
        ...
        print('fw')
        #print(xy[0,:,:])
        wire0 = torch.argmax(wg[0,:], dim=0)
        #print(wg[0,2675])
        #print(wire0)
        wg.register_hook(wire_hook)
        xy.register_hook(xy_hook)
