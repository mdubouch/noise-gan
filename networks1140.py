...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim, n_wires):
        ...
        self.lin0 = nn.Linear(latent_dims, seq_len * (n_features + 16), bias=False)
        self.convw = nn.Conv1d(16, n_wires, 1, 1, 0, bias=False)
    ...
    def forward(self, z):
        ...
        x = self.lin0(z).view(-1, n_features+16, self.seq_len)

        p = x[:,:n_features]
        w = self.convw(x[:,n_features:])
