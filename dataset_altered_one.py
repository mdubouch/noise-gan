...
class Data():
    ...
    def load(self):
        ...
        # data alteration
        loge = np.log(self.edep)
        # loge from 0 to 1
        norm_loge = (loge - loge.min()) / (loge.max() - loge.min())
        self.wire = np.clip(norm_loge * 3605 + np.random.randn(loge.size) * 10,
                0, 3605).astype(int)
        self.t = self.t * self.edep * 1e3
        self.t = self.t + self.t.min() + 1
        self.doca = self.doca * (-norm_loge**2 + 0.01 * np.random.randn(norm_loge.size))

        logging.info('Size wire %d pid %d doca %d' % (self.wire.size, self.pid.size, self.doca.size))

        self.edep = self.edep[:4*2048]
        self.t = self.t[:4*2048]
        self.doca = self.doca[:4*2048]
        self.wire = self.wire[:4*2048]
    ...
    def chunk(self, seq_len, batch_size):
        ...
        #train_minmax_chunked = train_minmax_chunked[:,:4]
        #wire_chunked = wire_chunked[:,:4]
