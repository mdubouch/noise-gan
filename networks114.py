...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.conv8 = nn.Conv1d(ngf, ngf, 9, 1, 4)

        self.convw = nn.Conv1d(ngf, encoded_dim, 9, 1, 4)
        self.convp = nn.Conv1d(ngf, n_features, 9, 1, 4)
    ...
    def forward(self, z, embed_space_noise=0.0, tau=1.0):
        ...
        return self.out(self.convp(x)), self.out(self.convw(x))
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv1 = nn.Conv1d(n_features+encoded_dim, ndf, 9, 1, 4)
        self.conv2 = nn.Conv1d(ndf, ndf*2, 9, 1, 4)

        self.lin0 = nn.Linear(ndf*2 * seq_len, 1, bias=True)
