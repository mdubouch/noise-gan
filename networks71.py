...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len):
        ...
        self.convw1 = nn.Conv1d(ngf*2, ngf*2, 513, 1, 256)
        self.convw2 = nn.Conv1d(ngf*2, ngf, 129, 1, 64)
        self.convw3 = nn.Conv1d(ngf*1, encoded_dim, 33, 1, 16)

        self.dec1 = nn.Conv1d(encoded_dim, ngf*1, 1, 1, 0)
        self.dec2 = nn.Conv1d(ngf*1, ngf*2, 1, 1, 0)
        self.dec3 = nn.Conv1d(ngf*2, n_wires, 1, 1, 0, bias=False)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len):
        ...
        self.enc1 = nn.Conv1d(n_wires, ndf, 1, 1, 0, bias=False)
        self.enc2 = nn.Conv1d(ndf, ndf*2, 1, 1, 0)
        self.enc3 = nn.Conv1d(ndf*2, encoded_dim, 1, 1, 0)
