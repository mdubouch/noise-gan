...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.conv7 = nn.Conv1d(ngf*4, ngf*3, 9, 1, 4)
        self.bn7 = nn.InstanceNorm1d(ngf*3)
        self.conv8 = nn.ConvTranspose1d(ngf*3, ngf*3, 9, 1, 4)
        self.bn8 = nn.InstanceNorm1d(ngf*3)

        self.convp1 = nn.ConvTranspose1d(ngf*3, ngf, 1, 1, 0)
        self.convp2 = nn.ConvTranspose1d(ngf*1, ngf, 1, 1, 0)
        self.convp3 = nn.ConvTranspose1d(ngf*1, ngf, 1, 1, 0)
        self.convp4 = nn.ConvTranspose1d(ngf, n_features, 1, 1, 0)

        self.convw1 = nn.ConvTranspose1d(ngf*3, ngf*3, 1, 1, 0)
        self.convw2 = nn.ConvTranspose1d(ngf*3, ngf*3, 1, 1, 0)
        self.convw3 = nn.ConvTranspose1d(ngf*3, ngf*3, 1, 1, 0)
        self.convw4 = nn.ConvTranspose1d(ngf*3, encoded_dim, 1, 1, 0)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convp1 = nn.Conv1d(n_features, ndf*1, 1, 1, 0)
        self.convp2 = nn.Conv1d(ndf, ndf, 1, 1, 0)
        self.convp3 = nn.Conv1d(ndf, ndf, 1, 1, 0)

        self.convw_diff1 = nn.Conv1d(encoded_dim, ndf*3, 1, 1, 0)
        self.convw1 = nn.Conv1d(encoded_dim, ndf*3, 1, 1, 0)
        self.convw_diff2 = nn.Conv1d(ndf*3, ndf*3, 1, 1, 0)
        self.convw2 = nn.Conv1d(ndf*3, ndf*3, 1, 1, 0)
        self.convw_diff3 = nn.Conv1d(ndf*3, ndf*3, 1, 1, 0)
        self.convw3 = nn.Conv1d(ndf*3, ndf*3, 1, 1, 0)

        self.conv1 = nn.Conv1d(ndf*4, ndf*8, 4, 2, 1)
        self.conv2 = nn.Conv1d(ndf*8, ndf*8, 4, 2, 1)
        self.conv3 = nn.Conv1d(ndf*8, ndf*8, 4, 2, 1)

        self.lin0 = nn.Linear(ndf*8 * seq_len // 8, 1, bias=True)
    ...
    def forward(self, x_): 
        ...
        w = self.act(w_diff + self.convw1(w))
        w_diff = self.convw_diff2(w - nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1]))
        w = self.act(w_diff + self.convw2(w))
        w_diff = self.convw_diff3(w - nn.ConstantPad1d((1, 0), 0.0)(w[:,:,:-1]))
        w = self.act(w_diff + self.convw3(w))
