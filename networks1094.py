...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim, n_wires):
        ...
        self.b1 = Simple(n512, n256, 5, 4, 1, output_padding=1)
        self.b2 = Simple(n256, n128, 5, 4, 1, output_padding=1)
        self.b3 = Simple(n128, n64, 5, 4, 1, output_padding=1)
        self.b4 = Simple(n64, n32, 5, 4, 1, output_padding=1)
        self.b5 = Simple(n32, n16, 3, 2, 1, output_padding=1)

        self.convp = nn.ConvTranspose1d(n16, n_features, 1, 1, 0, bias=True)
        self.convw = nn.ConvTranspose1d(n16, n_wires, 1, 1, 0, bias=True)
        self.out = nn.Tanh()
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim, n_wires):
        ...
        self.conv1 = nn.Conv1d(nproj+geom_dim+n_features, n64, 3, 1, 1, padding_mode='circular')
        self.ln1 = nn.LayerNorm((n64,seq_len), elementwise_affine=False)
        self.conv2 = nn.Conv1d(n64, n64, 3, 1, 3, dilation=3)
        self.ln2 = nn.LayerNorm((n64,seq_len), elementwise_affine=False)
        self.conv3 = nn.Conv1d(n64, n64, 3, 1, 9, dilation=9)
        self.ln3 = nn.LayerNorm((n64,seq_len), elementwise_affine=False)
        self.conv4 = nn.Conv1d(n64, n128, 3, 2, 1)
        self.ln4 = nn.LayerNorm((n128,seq_len//2), elementwise_affine=False)
        self.conv5 = nn.Conv1d(n128, n128, 3, 1, 1)
        self.ln5 = nn.LayerNorm((n128,seq_len//2), elementwise_affine=False)
        self.conv6 = nn.Conv1d(n128, n128, 3, 1, 3, dilation=3)
        self.ln6 = nn.Identity()#nn.LayerNorm((n128,seq_len//2), elementwise_affine=False)
    ...
    def forward(self, x_, xy_, w_): 
        ...
        x = self.ln1(self.act(self.conv1(x)))
        x = self.ln2(self.act(self.conv2(x)))
        x = self.ln3(self.act(self.conv3(x)))
        x = self.ln4(self.act(self.conv4(x)))
        x = self.ln5(self.act(self.conv5(x)))
        x = self.ln6(self.act(self.conv6(x)))
