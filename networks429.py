...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.conv1 = nn.ConvTranspose1d(1024, 512, 2, 2, 0)
        self.conv2 = nn.ConvTranspose1d(512, 256, 2, 2, 0)
        self.conv3 = nn.ConvTranspose1d(256, 128, 2, 2, 0)
        self.bn1 = nn.BatchNorm1d(512)
        self.bn2 = nn.BatchNorm1d(256)
        self.bn3 = nn.InstanceNorm1d(128)

        self.convw1 = nn.ConvTranspose1d(128, 64, 1, 1, 0)
        self.bnw1 = nn.InstanceNorm1d(64)
        self.convw2 = nn.ConvTranspose1d(64, n_wires, 1, 1, 0)

        #self.bnp0 = nn.BatchNorm1d(n_wires)
        self.convwp = nn.ConvTranspose1d(64, 32, 1, 1, 0)
        self.convp1 = nn.ConvTranspose1d(2, 32, 1, 1, 0)
        self.bnp1 = nn.InstanceNorm1d(64)
        self.convp2 = nn.ConvTranspose1d(64, 64, 1, 1, 0)
    ...
    def forward(self, z, wire_to_xy):
        ...
        wcode = self.act(self.bnw1(self.convw1(x)))
        w = self.convw2(wcode)
        wg = F.gumbel_softmax(w, dim=1, hard=True, tau=1/3)
        xy = torch.tensordot(wg, wire_to_xy, dims=[[1],[1]]).permute(0,2,1)

        p = self.act(self.bnp1(torch.cat([self.convwp(wcode), self.convp1(xy)], dim=1)))
        p = self.act(self.convp2(p))
        p = self.convp3(p)

        return torch.cat([self.out(p), xy, wcode], dim=1), wg
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convwcode = nn.Conv1d(64, 64, 1, 1, 0)
        self.convxy = nn.Conv1d(2, 64, 1, 1, 0)
        self.conv1 = nn.Conv1d(64*3, 256, 5, 1, 2)
        ...
        self.lin0 = nn.Linear(seq_len*512, 1)
    ...
    def forward(self, x_): 
        ...
        wcode = x[:,n_features+2:n_features+2+64]
        wg = x[:,n_features+2+64:]
        ...
        p = self.convp(p)
        xy = self.convxy(xy)
        wcode = self.convwcode(wcode)
        x = torch.cat([p, xy, wcode], dim=1)
