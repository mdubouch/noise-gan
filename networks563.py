...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        n64 = 64
        self.convp1 = nn.utils.spectral_norm(nn.Conv1d(n_features, n64, 1, 1, 0))
        self.convxy1 = nn.utils.spectral_norm(nn.Conv1d(geom_dim, n64, 1, 1, 0))
        self.convw1 = nn.utils.spectral_norm(nn.Conv1d(n_wires, n64, 1, 1, 0))
        ...
        self.conv4 = nn.utils.spectral_norm(nn.Conv1d(n256, n256, 3, 1, 1))
        self.conv5 = nn.utils.spectral_norm(nn.Conv1d(n256, n256, 3, 1, 1))
        ...
        self.lin0 = nn.utils.spectral_norm(nn.Linear(n256, 1))
    ...
    def forward(self, x_): 
        ...
        x = x0 + self.conv3(self.act(x))

        x0 = x
        x = self.conv4(self.act(x))
        x = x0 + self.conv5(self.act(x))
        x = self.act(x)
