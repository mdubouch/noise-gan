...
for e in range(pretrain_epochs):
    ...
    wires = torch.arange(gu.n_wires, device='cuda')
    n_its = 1
    for i in range(n_its):
        ...
        enc = enc + torch.randn_like(enc) * pretrain_noise_lvl
        # enc (256, enc_dim)
        fake_dist_matrix = enc.view(3606, 1, encoded_dim) - enc.view(1, 3606, encoded_dim)
        ...
        dist_loss = nn.L1Loss()(fake_dist_matrix, real_dist_matrix)
        ...
        loss = ae_loss + 10 * dist_loss + 5 * norm_loss
        loss.backward()
        optimizer_ae.step()
