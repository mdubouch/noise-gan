...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        ae_loss = F.mse_loss(fake_enc_w, fake_w)
        ...
        D_loss = -torch.mean(out_real) + torch.mean(out_fake) + lambda_gp * gp + 10 * ae_loss
        ...
        if (critic_count % n_critic == 0):
            ...
            ae_loss = F.mse_loss(fake_w, fake_enc_w)
            G_loss = -torch.mean(out_fake) + 10 * ae_loss
