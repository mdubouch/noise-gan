...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        class DBlock(nn.Module):
            def __init__(self, in_c, out_c, k_s, stride, pad):
                ...
                self.conv3 = nn.Conv1d(n64, n64, 5, 1, 2)
                self.conv4 = nn.Conv1d(n64+in_c, out_c, 1, 1, 0)
                self.act = nn.LeakyReLU(0.2)
            def forward(self, x):
                ...
                y = self.act(self.conv4(torch.cat([y, x], dim=1)))
                return y

        self.convgeom1 = nn.Conv1d(n64*2, n64*3, 9, 1, 4, padding_mode='circular')
        self.convgeom2 = nn.Conv1d(n64*3, n64*4, 9, 1, 8, dilation=2, padding_mode='circular')
        self.convgeom3 = nn.Conv1d(n64*4, n64*5, 9, 1, 16, dilation=4, padding_mode='circular')

        self.conv1 = nn.Conv1d(n64*6, n128, 9, 2, 4, padding_mode='circular')
        self.conv2 = nn.Conv1d(n128, n256, 5, 2, 2, padding_mode='circular')
        self.conv3 = nn.Conv1d(n256, n512, 3, 2, 1, padding_mode='circular')

        #self.b1 = DBlock(n128, n256, 5, 2, 0)
        #self.b2 = DBlock(n256, n512, 5, 2, 0)
        #self.b3 = DBlock(n512, n512, 5, 2, 0)
    ...
    def forward(self, x_, xy_, w_): 
        ...
        geom = torch.cat([w0, g0], dim=1)

        geom = self.act(self.convgeom1(geom))
        geom = self.act(self.convgeom2(geom))
        geom = self.convgeom3(geom)
