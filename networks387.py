...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.gb6 = GBlock(512, 512)

        self.convw = nn.ConvTranspose1d(512, n_wires, 1, 1, 0)

        #self.bnp0 = nn.BatchNorm1d(n_wires)
        self.convwp = nn.ConvTranspose1d(512, 64, 1, 1, 0)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        class DBlock(nn.Module):
            ...
            def forward(self, x):
                y0 = self.convp(F.interpolate(x, scale=0.5, mode='nearest'))
                y = self.convd(x))
                y = self.act(y0 + y)
                return y
