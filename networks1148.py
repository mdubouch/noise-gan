...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim, n_wires):
        ...
        self.s4 = Simple(128, 64, 3, 2, 1, output_padding=1)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim, n_wires):
        ...
        self.occ_mean = nn.Linear(1, 128)
        self.occ_logvar = nn.Linear(1, 128)

        self.w0 = nn.Linear(nproj+geom_dim, 128)
        self.w1 = nn.Linear(128, 128)

        self.p0 = nn.Linear(n_features, 128)
        self.p1 = nn.Linear(128, 128)

        self.s1 = Simple(256, 256, 3, 2, 1)
        self.s2 = Simple(256, 512, 3, 2, 1)
        self.s3 = Simple(512, 512, 3, 2, 1)
        self.s4 = Simple(512, 512, 3, 2, 1)
    ...
    def forward(self, x_, xy_, w_): 
        ...
        occupancy = wg.sum(dim=2).var(dim=1).unsqueeze(1) / 16.0 * 2.0 - 1.0
        _m = self.occ_mean(occupancy).unsqueeze(1)
        _lv = self.occ_logvar(occupancy).unsqueeze(1)

        w0 = self.convw0(wg)
        w = torch.cat([w0, xy], dim=1).permute(0,2,1)
        w = self.w1(self.act(self.w0(w)))
        w = (w - _m) / torch.exp(0.5 * _lv)

        p = self.p1(self.act(self.p0(p.permute(0,2,1))))

        x = torch.cat([p, w], dim=2).permute(0,2,1)
