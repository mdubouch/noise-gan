    ...
    def __init__(self, ngf, latent_dims, seq_len):
        ...
        class Decoder(nn.Module):
            def __init__(self):
                super().__init__()
                self.act = nn.ReLU()
                self.conv1 = nn.Conv1d(encoded_dim, encoded_dim, 1, 1, 0)
                self.conv2 = nn.Conv1d(encoded_dim, encoded_dim*2, 1, 1, 0)
                self.conv3 = nn.Conv1d(encoded_dim*2, encoded_dim*2, 1, 1, 0)
                self.conv4 = nn.Conv1d(encoded_dim*2, n_wires, 1, 1, 0)
            def forward(self, x):
                y = self.act(self.conv1(x))
                y1 = self.act(self.conv2(y + x))
                y = self.act(self.conv3(y1))
                y = self.conv4(y + y1)
                return y
        self.dec = Decoder()
    ...
    def __init__(self, ndf, seq_len):
        ...
        class Encoder(nn.Module):
            def __init__(self):
                super().__init__()
                self.act = nn.ReLU()
                self.conv1 = nn.Conv1d(n_wires, encoded_dim*2, 1, 1, 0)
                self.conv2 = nn.Conv1d(encoded_dim*2, encoded_dim*2, 1, 1, 0)
                self.conv3 = nn.Conv1d(encoded_dim*2, encoded_dim*1, 1, 1, 0)
                self.conv4 = nn.Conv1d(encoded_dim*1, encoded_dim, 1, 1, 0)
            def forward(self, x):
                y1 = self.act(self.conv1(x))
                y = self.act(y1 + self.conv2(y1))
