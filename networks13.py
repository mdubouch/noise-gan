...
class ResBlockTranspose(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.act(x + self.bn2(self.conv2(y)))
        if self.project_features:
...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len):
        ...
        self.resw0 = ResBlockTranspose(ngf*2, ngf*2, 129, 1, 64)
        self.resw1 = ResBlockTranspose(ngf*2, ngf*2, 257, 1, 128)
        self.resw2 = ResBlockTranspose(ngf*2, n_wires, 513, 1, 256)

        self.resp0 = ResBlockTranspose(ngf*2, ngf*2, 129, 1, 64)
        self.resp1 = ResBlockTranspose(ngf*2, ngf*2, 257, 1, 128)
        self.resp2 = ResBlockTranspose(ngf*2, n_features, 513, 1, 256)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len):
        ...
        self.resw1 = ResBlock(ndf*2, ndf*2, 257, 1, 128, padding_mode='circular')
        self.resw2 = ResBlock(ndf*2, ndf*2, 129, 1, 64, padding_mode='circular')

        self.resp0 = ResBlock(n_features, ndf*2, 513, 1, 256, padding_mode='circular')
        self.resp1 = ResBlock(ndf*2, ndf*2, 257, 1, 128, padding_mode='circular')
        self.resp2 = ResBlock(ndf*2, ndf*2, 129, 1, 64, padding_mode='circular')
        

        self.res1 = ResBlock(ndf*4, ndf*4, 17, 1, 8, padding_mode='circular')
        self.res2 = ResBlock(ndf*4, ndf*4, 17, 1, 8, padding_mode='circular')
        self.res3 = ResBlock(ndf*4, ndf*6, 17, 1, 8, padding_mode='circular')
