...
wire_to_xy_norm = (wire_to_xy - wire_to_xy * (wire_to_r.min()-10) / wire_to_r) / (wire_to_r.max() - wire_to_r.min() + 10)
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        real_x = concatenate_p_w_xy(real_p, real_xy, 0)
        out_real = disc(real_x)

        fake_x, _ = sample_fake(real_p.shape[0], tau)
        #fake_p = fake_x[:,:3]
        #fake_xy = fake_x[:,3:]
