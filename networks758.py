...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv0 = nn.Conv1d(geom_dim, 1024, 1, 1, 0)
        self.conv1 = nn.Conv1d(1024, n128, 17, 2, 8, padding_mode='circular')
    ...
    def forward(self, x_, w_, wire_sphere): 
        ...
        x = self.act(self.conv2(self.dropout(x)))
        x = self.act(self.conv3(self.dropout(x)))
        x = self.act(self.conv4(self.dropout(x)))
        x = self.act(self.conv5(self.dropout(x)))
