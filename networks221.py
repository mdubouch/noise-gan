...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.lin0 = nn.Linear(latent_dims, seq_len//1*ngf*8, bias=True)


        self.conv0 = nn.Conv1d(ngf*8, ngf*8, 1, 1, 0)
        self.bn0 = nn.InstanceNorm1d(ngf*8)
        self.conv1 = nn.ConvTranspose1d(ngf*8, ngf*4, 1, 1, 0)
        self.bn1 = nn.InstanceNorm1d(ngf*4)
        self.conv2 = nn.ConvTranspose1d(ngf*4, ngf*2, 1, 1, 0)
        self.bn2 = nn.InstanceNorm1d(ngf*2)
        self.conv3 = nn.ConvTranspose1d(ngf*2, ngf*1, 1, 1, 0)
        self.bn3 = nn.InstanceNorm1d(ngf*1)
        self.conv4 = nn.ConvTranspose1d(ngf*1, ngf*1, 1, 1, 0)
        self.bn4 = nn.InstanceNorm1d(ngf*1)
        self.conv5 = nn.ConvTranspose1d(ngf*1, ngf*1, 1, 1, 0)
    ...
    def forward(self, z, embed_space_noise=0.0, tau=1.0):
        ...
        x = self.act(self.lin0(z).view(-1, self.ngf*8, self.seq_len // 1))
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv3 = nn.Conv1d(ndf*1, ndf*8, 1, 1, 0)
        self.conv4 = nn.Conv1d(ndf*8, ndf*8, 1, 1, 0)
        self.conv5 = nn.Conv1d(ndf*8, ndf*8, 1, 1, 0)
        
        self.lin0 = nn.Linear(ndf*8 * seq_len // 1, 1, bias=True)
    ...
    def __init__(self, encoded_dim):
        ...
        class Enc(nn.Module):
            def __init__(self, hidden_size):
                ...
                self.lin5 = nn.Linear(hidden_size, encoded_dim)
                self.out = nn.Tanh()
            def forward(self, x):
                y = self.lin5(self.act(self.lin4(self.act(self.lin3(self.act(self.lin2(self.act(self.lin1(x)))))))))
                return self.out(y)
        ...
        class Dec(nn.Module):
            def __init__(self, hidden_size):
                ...
                self.lin3 = nn.Linear(hidden_size, hidden_size)
                self.lin4 = nn.Linear(hidden_size, hidden_size)
                self.lin5 = nn.Linear(hidden_size, n_wires)
            def forward(self, x):
                y = self.lin5(self.act(self.lin4(self.act(self.lin3(self.act(self.lin2(self.act(self.lin1(x)))))))))
                return y
        self.enc_net = Enc(encoded_dim*8)
        self.dec_net = Dec(encoded_dim*8)
