    ...
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        class GBlock(nn.Module):
            ...
            def forward(self, x):
                ...
                y = self.act(y0 + self.bn1(self.conv1(y0)))
                return y

        self.gb1 = GBlock(512, 512)
        self.gb2 = GBlock(512, 512)
        self.gb3 = GBlock(512, 512)
        self.gb4 = GBlock(512, 256)
        self.gb5 = GBlock(256, 128)

        self.gbp1 = GBlock(128, 64)
        self.convp1 = nn.ConvTranspose1d(64, 64, 3, 1, 1)
        self.convp2 = nn.ConvTranspose1d(64, n_features, 1, 1, 0)

        self.gbw1 = GBlock(128, 64)
        self.convw1 = nn.ConvTranspose1d(64, 64, 3, 1, 1)
        self.convw2 = nn.ConvTranspose1d(64, encoded_dim, 1, 1, 0)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.dbp3 = DBlock(256, 256)

        self.dbw1 = DBlock(64, 128)
        self.dbw2 = DBlock(128, 256)
        self.dbw3 = DBlock(256, 256)
    ...
    def forward(self, x_): 
        ...
        p0 = self.act(self.convp(p))
        p = self.act(self.dbp1(p0))
        p = self.act(self.dbp2(p))
        p = self.act(self.dbp3(p))

        w0 = self.act(self.convw(w))
        w = self.act(self.dbw1(w0))
        w = self.act(self.dbw2(w))
        w = self.act(self.dbw3(w))

        #x = F.dropout(torch.cat([p, w], dim=1), p=0.5)
        x = torch.cat([p, w], dim=1)

        x0 = x
        x = self.act(self.db1(x0))
        x = self.act(self.db2(x))
