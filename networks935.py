...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convw1 = nn.Conv1d(nproj, nproj, 1, 1, 0)
        self.convw2 = nn.Conv1d(nproj, n64, 3, 1, 1, padding_mode='circular')
        ...
        nbranch = n64*3
        self.conv1 = nn.Conv1d(nbranch, nbranch*2, 3, 1, 1, padding_mode='circular')
        self.conv2 = nn.Conv1d(nbranch*2, nbranch*4, 3, 1, 2, dilation=2, padding_mode='circular')
        self.conv3 = nn.Conv1d(nbranch*4, nbranch*8, 3, 1, 4, dilation=4, padding_mode='circular')

        self.lin0 = nn.Linear(nbranch*8, 1)
        self.dropout = nn.Dropout(0.1)
    ...
    def forward(self, x_, xy_, w_): 
        ...
        p = self.act(self.convp1(p0))

        w0 = self.convw0(wg)
        w1 = self.act(self.convw1(w0))
        w = self.act(self.convw2(w0))

        g0 = self.act(self.convg0(xy))
        g = self.act(self.convg1(g0))

        x0 = torch.cat([p, w, g], dim=1)

        x1 = self.act(self.conv1(self.dropout(x0)))
        x2 = self.act(self.conv2(x1))
        x = self.act(self.conv3(x2))
