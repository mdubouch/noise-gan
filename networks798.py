...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convw1 = nn.Conv1d(n64, n_wires, 3, 1, 1)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv0 = nn.Conv1d(n_wires, n64, 1, 1, 0)
        self.conv1 = nn.Conv1d(n64, n256, 3, 1, 1)
        self.bn1 = nn.InstanceNorm1d(n256)
        self.conv2 = nn.Conv1d(n256, n128, 3, 1, 1)
        self.bn2 = nn.InstanceNorm1d(n128)
        self.conv3 = nn.Conv1d(n128, n64, 3, 1, 1)
        self.bn3 = nn.InstanceNorm1d(n64)
        self.lin2 = nn.Linear(n64, 1)
    ...
    def forward(self, x_, w_, wire_sphere): 
        ...
        x = self.act(self.bn3(self.conv3(x))).permute(0,2,1)

        x = self.lin2(x.mean(dim=1)).squeeze()
