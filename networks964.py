...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convw1 = DBlock(nproj, n64, 3, 1, 1, padding_mode='circular')
        self.convw2 = DBlock(nproj+n64, n128, 3, 2, 1, padding_mode='circular')
        self.convw3 = DBlock(nproj+n64+n128, n256, 3, 2, 1, padding_mode='circular')

        self.convg0 = nn.Conv1d(geom_dim, nproj, 1, 1, 0)
        self.convg1 = DBlock(nproj, n64, 3, 1, 1, padding_mode='circular')
        self.convg2 = DBlock(nproj+n64, n128, 3, 2, 1, padding_mode='circular')
        self.convg3 = DBlock(nproj+n64+n128, n256, 3, 2, 1, padding_mode='circular')

        self.convp0 = nn.Conv1d(n_features, nproj, 1, 1, 0)
        self.convp1 = DBlock(nproj, n64, 3, 1, 1, padding_mode='circular')
        self.convp2 = DBlock(nproj+n64, n128, 3, 2, 1, padding_mode='circular')
        self.convp3 = DBlock(nproj+n64+n128, n256, 3, 2, 1, padding_mode='circular')

        # Concatenated branch
        ncat = nproj*3
        self.conv1 = DBlock(ncat, ncat*2, 3, 1, 1, padding_mode='circular')
        self.conv2 = DBlock(ncat+ncat*2, ncat*4, 3, 2, 1, padding_mode='circular')
        self.conv3 = DBlock(ncat+ncat*2+ncat*4, ncat*8, 3, 2, 1, padding_mode='circular')

        # Common branch
        ncom =(nproj+n64+n128+n256)*3 + ncat+ncat*2+ncat*4+ncat*8
        self.convc1 = nn.Conv1d(ncom, ncom, 3, 1, 1, padding_mode='circular')
        self.convc2 = nn.Conv1d(ncom, ncom, 3, 1, 1, padding_mode='circular')
        self.convc3 = nn.Conv1d(ncom, ncom, 3, 1, 1, padding_mode='circular')

        self.lin0 = nn.Linear((nproj+n64+n128+n256)*3 + ncat+ncat*2+ncat*4+ncat*8, 1)
        self.dropout = nn.Dropout(0.1)
    ...
    def forward(self, x_, xy_, w_): 
        ...
        x = self.convc1(self.dropout(x0))
        x = x0 + self.convc2(self.act(x))
        x = self.act(self.convc3(self.act(x)))
