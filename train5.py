...
wire_to_xy = wire_to_xy / gu.wire_x.max()
...
for e in range(n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        real_wx = wire_to_xy[:,real_w].squeeze(2).permute(1, 0, 2)

        out_real = torch.sigmoid(disc(add_noise(real_p, noise_level, -1, 1), real_wx))
        D_loss_real = F.binary_cross_entropy(out_real, torch.ones_like(out_real))

        fake_p, fake_w = sample_fake(real_p.shape[0], tau)
        fake_wx = torch.tensordot(wire_to_xy, fake_w, dims=([1], [1])).permute(1, 0, 2)

        out_fake = torch.sigmoid(disc(fake_p.detach(), fake_wx.detach()))
        D_loss_fake = F.binary_cross_entropy(out_fake, torch.zeros_like(out_fake))
        ...
        if (i % n_critic == 0):
            ...
            fake_w = fake_hits[1]
            fake_wx = torch.tensordot(wire_to_xy, fake_w, dims=([1], [1])).permute(1, 0, 2)
            out_fake = torch.sigmoid(disc(fake_p, fake_wx))


            # diagnostic
            #__fw = torch.argmax(fake_w[0,:,0])
            #print(__fw)
            #print(gu.wire_x[__fw], gu.wire_y[__fw])
            #print(fake_wx[0,:,0])

            occupancy_loss = (fake_w.sum(2).var(1).mean() - real_w.float().sum(2).var(1).mean())**2
            occupancy_losses.append(occupancy_loss.item())
            
            G_loss = F.binary_cross_entropy(out_fake, torch.ones_like(out_fake))
