...
optimizer_disc = torch.optim.Adam(disc.parameters(), lr=1e-4, betas=(0.0, 0.9), 
        weight_decay=0)
...
def gradient_penalty(disc, real_p, real_w, fake_p, fake_w):
    ...
    interpolates_w = real_w.requires_grad_(True)
    if (eps.mean() < 0.5):
    ...
    gradient_pen = ((gradients_p.norm(2, dim=1) - 1)**2).mean() + ((gradients_w.norm(2, dim=1) - 1)**2).mean()
    return gradient_pen
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        out_real = disc(add_noise(real_p, noise_level, -1, 1), real_wx)
        ...
        out_fake = disc(fake_p.detach(), fake_wx.detach())
        D_loss_fake = out_fake

        gp = gradient_penalty(disc, add_noise(real_p, noise_level, -1, 1), real_wx, 
                              fake_p, fake_wx)
