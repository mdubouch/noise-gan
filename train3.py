...
train_loader, train_dataset, n_chunks = data.chunk(seq_len, batch_size=48)
...
for e in range(n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        if (i % n_critic == 0):
            ...
            #print('real', fake_w.sum(dim=2).var(1))
            #print('fake', real_w.sum(dim=2).var(1))
            #occupancy_loss = torch.mean(((fake_w.sum(dim=2).var(1) - real_w.sum(dim=2).var(1)) / real_w.sum(dim=2).var(1))**2)
            #print(occupancy_loss.item())
            #print('g loss:', -torch.mean(out_fake))
            #print('real:', real_w.sum(2).var(1).mean())
            #print('fake:', fake_w.sum(2).var(1).mean())
            occupancy_loss = 0.1 * (fake_w.sum(2).var(1).mean() - real_w.sum(2).var(1).mean())**2
            occupancy_losses.append(occupancy_loss.item())
            critic_loss = -torch.mean(out_fake)
            G_loss = critic_loss + occupancy_loss    

            generator_losses.append(G_loss.item())
            #print(fake_w[0].shape)
            #print(fake_w[0])
            #print(fake_w.sum(2)[0])
            #print(fake_w.sum(2).mean(0))
            #print(real_w.sum(2).mean(0))
            if i % 10 == 0:
                logging.info('gen critic loss: %f' % (critic_loss))
                logging.info('gen occupancy loss: %f' % (occupancy_loss))
