...
for e in range(pretrain_epochs):
    ...
    for i in range(n_its):
        ...
        reenc = ae.enc_net(F.gumbel_softmax(encdec, dim=1, hard=False))
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        real_x = concatenate_p_w_xy(real_p, real_w_ohe, 0)
        ...
        #ae_loss = F.cross_entropy(real_dec_w, real_w.squeeze().cuda()) + 2 * F.mse_loss(real_reenc_w, real_enc_w)
        #ae_loss.backward()
        #print(ae_loss.item())
        #optimizer_ae.step()
        ...
        if (critic_count % n_critic == 0):
            ...
            fake_w = fake_hits[1]
            fake_dec_w = F.gumbel_softmax(ae.dec(fake_w), dim=1, hard=False, tau=tau)
            fake_x = concatenate_p_w_xy(fake_p, fake_dec_w, 0)
            out_fake = disc(fake_x)
