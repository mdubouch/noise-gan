...
parser.add_argument('--local_rank', type=int) # For distributed multi-GPU training
...
torch.distributed.init_process_group(backend='nccl', world_size=n_gpus)
gen = nn.parallel.DistributedDataParallel(networks.Gen(ngf=ngf, latent_dims=latent_dims, seq_len=seq_len, 
    encoded_dim=encoded_dim, n_wires=gu.n_wires).cuda(args.local_rank), 
    device_ids=[args.local_rank], output_device=args.local_rank)
logging.info(gen)
disc = nn.parallel.DistributedDataParallel(networks.Disc(ndf=ndf, seq_len=seq_len, encoded_dim=encoded_dim, 
    n_wires=gu.n_wires).cuda(args.local_rank),
    device_ids=[args.local_rank], output_device=args.local_rank)
logging.info(disc)
...
train_loader, train_dataset, n_chunks = data.chunk(seq_len, batch_size=args.batch_size, 
        num_replicas=n_gpus, rank=args.local_rank)
...
# TODO remove r (?)
wire_sphere[2] = torch.from_numpy(np.sqrt(gu.wire_x**2+gu.wire_y**2))
...
def save_states(epoch):
    states = { 'disc': disc.module.state_dict(), 'd_opt': optimizer_disc.state_dict(), 
            'd_loss': discriminator_losses, 'gen': gen.module.state_dict(), 
...
def gradient_penalty(disc, interp_p, interp_xy, interp_w):
    interp_p.requires_grad_()
    interp_xy = interp_xy.detach()
    interp_xy.requires_grad_()
    interp_w.requires_grad_()


    d_interpolates = disc(interp_p, interp_xy, interp_w).squeeze()
    ...
    gradients_xy = autograd.grad(outputs=d_interpolates,
            inputs=interp_xy,
            grad_outputs=grad_outputs_w,
            create_graph=True, # IMPORTANT! Allows to compute gradient with respect to gradient
            only_inputs=False
    ...
    gradients_xy_pen = (gradients_xy.pow(2).flatten(1, 2).sum(dim=1).pow(0.5) - 1)**2

    gradient_pen = torch.mean(gradients_p_pen + gradients_xy_pen + gradients_w_pen)
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        out_real = disc(real_p, real_xy, real_w_ohe)
        ...
        out_fake = disc(fake_p, fake_xy, fake_wg)
        ...
        gp = gradient_penalty(disc, interpolates_p, interpolates_xy, interpolates_w)
        ...
        if (critic_count % n_critic == 0):
            ...
            optimizer_disc.zero_grad()
            # Calculate validation loss once every G step
            val_p, val_w = sample_test()
            val_w_ohe = F.one_hot(val_w.cuda(), 
                num_classes=gu.cum_n_wires[-1]).squeeze(1).permute(0, 2, 1).float()
            val_p = val_p.unsqueeze(0).cuda()
            val_xy = torch.tensordot(val_w_ohe, wire_sphere, dims=[[1], [1]]).permute(0,2,1)
            out_real = disc(val_p, val_xy, val_w_ohe)
            ...
            validation_losses.append(D_loss.item())
