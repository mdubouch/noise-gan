...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convp2 = GBlock(n512, n64, 3, 1, 1)
    ...
    def forward(self, z, wire_to_xy):
        ...
        x = self.act(self.lin0(z).reshape(-1, self.n512, self.seq_len // 1))
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv1 = nn.Conv1d(n512, n512, 1, 1, 0)
        self.lin1 = nn.Linear(n512*1, n256)
        self.lin2 = nn.Linear(n256, 1)
        #self.conv0 = nn.Conv1d(geom_dim, n512, 1, 1, 0)
    ...
    def forward(self, x_, w_, wire_sphere): 
        ...
        x = self.act(self.lin1(x.permute(0, 2,1)))
        x = self.lin2(x).squeeze().sum(dim=1)
