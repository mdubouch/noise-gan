...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len):
        ...
        self.convw0 = nn.ConvTranspose1d(ngf*2, ngf*2, 129, 1, 64)
        ...
        self.convw4 = nn.ConvTranspose1d(ngf*2, ngf*1, 769, 1, 384)
        ...
        self.convp2 = nn.ConvTranspose1d(ngf*2, ngf*2, 17, 1, 8)
        self.bnp2 = nn.BatchNorm1d(ngf*2)
        self.convp3 = nn.ConvTranspose1d(ngf*2, ngf*1, 17, 1, 8)
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len):
        ...
        self.convw0 = nn.utils.spectral_norm(nn.Conv1d(ndf*1, ndf*2, 769, 1, 384, padding_mode='circular'))
        self.convw1 = nn.utils.spectral_norm(nn.Conv1d(ndf*2, ndf*2, 513, 1, 256, padding_mode='circular'))
        self.convw2 = nn.utils.spectral_norm(nn.Conv1d(ndf*2, ndf*2, 257, 1, 128, padding_mode='circular'))
        self.convw3 = nn.utils.spectral_norm(nn.Conv1d(ndf*2, ndf*2, 129, 1, 64, padding_mode='circular'))
        self.convw4 = nn.utils.spectral_norm(nn.Conv1d(ndf*2, ndf*2, 129, 1, 64, padding_mode='circular'))
        ...
        self.conv1 = nn.utils.spectral_norm(nn.Conv1d(ndf*4+1, ndf*4, 17, 1, 8, padding_mode='circular'))
    ...
    def forward(self, p_, w_): # p_ shape is (batch, features, seq_len), w_ shape is one-hot encoded wire (batch, 4986, seq_len)
        ...
        x = torch.cat([p, w, occupancy], dim=1)
