...
for i in range(gu.n_layers):
    ...
    wire_sphere[0, start:end] = torch.from_numpy(np.ones_like(x) * i)
    wire_sphere[1, start:end] = torch.from_numpy(np.arctan2(y, x))
...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        print('out real %.1f' % out_real.mean())
        ...
        print('out fake %.1f' % out_fake.mean())
