...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.conv41 = nn.Conv1d(n512, 1024, 3, 2, 0)
        self.conv42 = nn.Conv1d(1024, n512, 1, 1, 0)
        self.conv51 = nn.Conv1d(n512, 1024, 3, 2, 0)
        self.conv52 = nn.Conv1d(1024, n512, 1, 1, 0)
    ...
    def forward(self, x_, w_, wire_sphere): 
        ...
        x3 = self.conv3(self.dropout(self.act(x2)))
        x = self.conv41(self.dropout(self.act(x3)))
        x = self.conv42(self.dropout(self.act(x)))
        x = self.conv51(self.dropout(self.act(x)))
        x = self.conv52(self.dropout(self.act(x)))
