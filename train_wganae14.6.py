...
for e in range(start_epoch, start_epoch + n_epochs):
    ...
    for i, (real_p, real_w) in enumerate(train_loader):
        ...
        fake_dist_matrix = torch.sqrt(1e-8 + (fake_dist_matrix**2).sum(dim=2))
        ...
        fake_x = concatenate_p_w(fake_p, fake_dec_w).detach()
        ...
        interpolates_w = F.gumbel_softmax(ae.dec(interpolates_enc_w), dim=1, hard=True, tau=tau)
        ...
        if (i % n_critic == 0):
            ...
            fake_w = fake_hits[1]
            fake_dec_w = F.gumbel_softmax(ae.dec(fake_w), dim=1, hard=True, tau=tau)
            fake_x = concatenate_p_w(fake_p, fake_dec_w)
