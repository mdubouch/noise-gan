...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.gbw4 = GBlock(n64, n64, 3, 1, 1)
        self.gbw3 = GBlock(n64, n64, 33, 1, 16)
        self.gbw2 = GBlock(n64, n64, 129, 1, 64)
    ...
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        class DBlock(nn.Module):
            def __init__(self, in_c, out_c, k_s, stride, pad):
                super().__init__()
                self.conv1 = nn.Conv1d(in_c, 128, 1, 1, 0)
                self.conv2 = nn.Conv1d(128, 128, k_s, stride, pad)
                self.conv3 = nn.Conv1d(128, 128, 5, 1, 2)
                self.conv4 = nn.Conv1d(128+in_c, out_c, 1, 1, 0)
                self.act = nn.LeakyReLU(0.2)
            def forward(self, x):
