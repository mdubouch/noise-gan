    ...
    def __init__(self, ndf, seq_len, encoded_dim, n_wires):
        ...
        class ResBlockDown(nn.Module):
            def __init__(self, channels):
                ...
                self.conv2 = nn.Conv1d(channels*2, channels, 3, 1, 1, padding_mode='circular')
                self.convp = nn.Conv1d(channels, channels, 2, 2, 0, bias=False)
                self.act = nn.LeakyReLU(0.2)
            def forward(self, x):
                y = self.act(self.conv1(x))
                y = self.conv2(y)
                xp = self.convp(x)
                y = self.act(torch.cat([y, xp], dim=1))
                return y
