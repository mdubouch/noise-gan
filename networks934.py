...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convw2 = nn.Conv1d(nproj, n64, 3, 1, 2, dilation=2, padding_mode='circular')
        self.convw  = nn.Conv1d(nproj+n64*2, n64*4, 3, 1, 1, padding_mode='circular')

        self.convg0 = nn.Conv1d(geom_dim, nproj, 1, 1, 0, bias=False)
        self.convg1 = nn.Conv1d(nproj, n64, 3, 1, 1, padding_mode='circular')
        self.convg2 = nn.Conv1d(nproj, n64, 3, 1, 2, dilation=2, padding_mode='circular')
        self.convg  = nn.Conv1d(nproj+n64*2, n64*4, 3, 1, 1, padding_mode='circular')

        self.convp0 = nn.Conv1d(n_features, nproj, 1, 1, 0, bias=False)
        self.convp1 = nn.Conv1d(nproj, n64, 3, 1, 1, padding_mode='circular')
        self.convp2 = nn.Conv1d(nproj, n64, 3, 1, 2, dilation=2, padding_mode='circular')
        self.convp  = nn.Conv1d(nproj+n64*2, n64*4, 3, 1, 1, padding_mode='circular')

        nbranch = n64*4*3
        self.conv1 = nn.Conv1d(nbranch, n64*16, 3, 1, 1, padding_mode='circular')
        self.conv2 = nn.Conv1d(nbranch, n64*16, 3, 1, 2, dilation=2, padding_mode='circular')
        self.conv3 = nn.Conv1d(nbranch, n64*16, 3, 1, 4, dilation=4, padding_mode='circular')

        self.lin0 = nn.Linear(n64*16*3, 1)
        self.dropout = nn.Dropout(0.1)
    ...
    def forward(self, x_, xy_, w_): 
        ...
        p1 = self.convp1(p0)
        p2 = self.convp2(p0)
        p = self.convp(self.act(torch.cat([p0, p1, p2], dim=1)))

        w0 = self.convw0(wg)
        w1 = self.convw1(w0)
        w2 = self.convw2(w0)
        w = self.convw(self.act(torch.cat([w0, w1, w2], dim=1)))

        g0 = self.convg0(xy)
        g1 = self.convg1(g0)
        g2 = self.convg2(g0)
        g = self.convg(self.act(torch.cat([g0, g1, g2], dim=1)))

        x0 = torch.cat([p, w, g], dim=1)
        x0 = self.act(x0)

        x1 = self.conv1(self.dropout(x0))
        x2 = self.conv2(self.dropout(x0))
        x3 = self.conv3(self.dropout(x0))
