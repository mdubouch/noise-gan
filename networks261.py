...
class Gen(nn.Module):
    def __init__(self, ngf, latent_dims, seq_len, encoded_dim):
        ...
        self.convp4 = nn.ConvTranspose1d(128, n_features, 1, 1, 0)

        self.convw1 = nn.ConvTranspose1d(128, 128, 1, 1, 0)
        self.bnw1 = nn.InstanceNorm1d(128)
        self.convw2 = nn.ConvTranspose1d(128, 128, 1, 1, 0)
        self.bnw2 = nn.InstanceNorm1d(128)
        self.convw3 = nn.ConvTranspose1d(128, 128, 1, 1, 0)
        self.convw4 = nn.ConvTranspose1d(128, encoded_dim, 1, 1, 0)
    ...
    def forward(self, z, embed_space_noise=0.0, tau=1.0):
        ...
        w = x
        w0 = w
        w = self.act(self.bnw1(self.convw1(w)))
        w = self.act(w0 + self.bnw2(self.convw2(w)))
        w0 = w
...
class Disc(nn.Module):
    def __init__(self, ndf, seq_len, encoded_dim):
        ...
        self.convp1 = nn.Conv1d(n_features, 128, 1, 1, 0)
        self.convp2 = nn.Conv1d(128, 128, 1, 1, 0)
        self.convp3 = nn.Conv1d(128, 128, 1, 1, 0)
        self.convp4 = nn.Conv1d(128, 128, 1, 1, 0)
        self.convw1 = nn.Conv1d(encoded_dim, 128, 1, 1, 0)
        self.convw2 = nn.Conv1d(128, 128, 1, 1, 0)
        self.convw3 = nn.Conv1d(128, 128, 1, 1, 0)
        self.convw4 = nn.Conv1d(128, 128, 1, 1, 0)

        self.conv1 = nn.Conv1d(256, 256, 4, 2, 1)
        self.conv2 = nn.Conv1d(256, 512, 4, 2, 1)
        self.conv3 = nn.Conv1d(512, 512, 4, 2, 1)
        self.conv4 = nn.Conv1d(512, 512, 4, 2, 1)
        
        self.lin0 = nn.Linear(512 * seq_len // 16, 1, bias=True)
