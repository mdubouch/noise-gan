...
print('Import networks version %s' % (args.net_version))
logging.info('networks=%s' % (args.net_version))
...
wire_sphere[0] = torch.from_numpy(np.flip(gu.wire_x).copy())
wire_sphere[1] = torch.from_numpy(np.flip(gu.wire_y).copy())
wire_r = torch.sqrt(wire_sphere[0]**2 + wire_sphere[1]**2)
wire_th = torch.atan2(wire_sphere[1], wire_sphere[0])

wire_cylinder = torch.zeros((3, gu.n_wires), device='cuda')
...
for i in range(gu.n_layers):
    ...
    #print(dist_between_wires)
    print('avg theta dist between wires:', np.mean(theta_dist))
    if i > 0:
    ...
    wire_cylinder[0,start:end] = x
    wire_cylinder[1,start:end] = y
    wire_cylinder[2,start:end] = z
    wire_cylinder[:,start:end] /= wire_cylinder[:,start:end].norm(dim=0, keepdim=True)
wire_sphere = wire_cylinder
...
#wire_sphere = (wire_sphere - smin) / (smax - smin + 1e-8) * 2 - 1
logging.info('DISTANCE BETWEEN WIRES: {}'.format(torch.norm(wire_sphere[:,1:] - wire_sphere[:,:-1], dim=0)))
print('DISTANCE BETWEEN WIRES: {}'.format(torch.norm(wire_sphere[:,1:] - wire_sphere[:,:-1], dim=0).mean()))
